  ...
  <div class="employee-page">
    ...
    <div class="filter-bar d-flex flex-wrap gap-2 align-items-center mb-4">
      <div class="search-wrap">
        ...
        <input v-model="search" type="text" class="form-control search-input" placeholder="Search employees..." />
      </div>
    ...
    <div v-else-if="filteredEmployees.length" class="row g-3">
      <div class="col-12 col-md-6 col-lg-4" v-for="emp in filteredEmployees" :key="emp.id">
        <div class="emp-card">
          ...
          <div class="emp-details">
            ...
            <div class="detail-row"><i class="bi bi-building"></i> {{ branchName(emp.branchId) }}</div>
          </div>
...
const positions = ['Store Manager', 'Barista', 'Cashier', 'Kitchen Staff', 'Cleaning Staff', 'Supervisor', 'Delivery Staff', 'Server']
...
const filteredEmployees = computed(() => {
  return employees.value.filter(e => {
    ...
    const matchBranch = !filterBranch.value || e.branchId === filterBranch.value
...
const stats = computed(() => {
  ...
  const managers = employees.value.filter(e => e.position === 'Store Manager').length
  return [
    { label: 'Total Employees', value: employees.value.length, sub: 'All branches', icon: 'bi bi-people' },
    { label: 'Active', value: active, sub: 'Currently working', icon: 'bi bi-person-check' },
    { label: 'Part-Time', value: employees.value.filter(e => e.hourlyRate < 80).length, sub: 'Hourly workers', icon: 'bi bi-clock' },
...
const validate = () => {
  const e = {}
  if (!form.value.firstName.trim() || !form.value.lastName.trim()) e.name = 'Full name is required.'
  ...
  if (!form.value.hourlyRate || form.value.hourlyRate <= 0) e.hourlyRate = 'Hourly rate must be greater than 0.'
...
.employee-page { padding: 1.5rem; background: #f4f1ef; min-height: 100vh; }
