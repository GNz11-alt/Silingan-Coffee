  ...
  <div class="maintenance-page">
    ...
    <div v-if="activeTab === 'overview'" class="tab-content">
        ...
        <div class="panel">
          ...
          <div class="resource-list">
            <div class="resource-row">
              ...
              <span class="resource-value">{{ storage.used }}GB / {{ storage.total }}GB</span>
            </div>
            ...
            <div class="progress-bar-wrap">
              <div class="progress-bar" :style="{ width: memoryPercent + '%', background: memoryPercent > 80 ? '#dc3545' : '#532f15' }"></div>
            </div>
          </div>
        </div>
      ...
      <div class="alert-panel mt-3" v-if="showOptimizationAlert">
        ...
        <div>
          ...
          <div class="alert-sub">Consider running database optimization to improve performance. Last optimization was {{ lastOptimization }}.</div>
        </div>
    ...
    <div v-if="activeTab === 'backup'" class="tab-content">
      <div class="panel">
        ...
        <p class="panel-sub">Create and manage database backups for Silingan Coffee system.</p>
          ...
          <table class="history-table">
            ...
            <tbody>
              <tr v-for="b in backupHistory" :key="b.id">
                ...
                <td>{{ b.type }}</td>
                <td>{{ b.size }}</td>
                <td><span class="badge" :class="b.status === 'Success' ? 'badge-active' : 'badge-inactive'">{{ b.status }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <table v-else class="history-table">
          ...
          <tbody>
            ...
            <tr v-if="!userSessions.length">
              <td colspan="4" class="text-center text-muted py-4" style="font-size:13px">No active sessions.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="panel">
        ...
        <div class="settings-list">
          <div class="setting-row">
            <div>
              <div class="setting-label">Low Stock Threshold (default)</div>
              <div class="setting-sub">Items below this quantity will trigger low stock alerts</div>
            </div>
            <input v-model.number="settings.lowStockThreshold" type="number" class="setting-input" min="1" />
          </div>
          <div class="setting-row">
            <div>
              <div class="setting-label">System Timezone</div>
              <div class="setting-sub">All timestamps will use this timezone</div>
            </div>
          ...
          <div class="setting-row">
            <div>
              ...
              <div class="setting-sub">Automatically backup the database daily</div>
            </div>
            <label class="toggle">
          ...
          <div class="setting-row">
            <div>
              ...
              <div class="setting-sub">Restrict access to admins only during maintenance</div>
            </div>
            <label class="toggle">
...
import {
  ...
  Shield, AlertTriangle, Download, Settings, CheckSquare
...
const lastBackupStatus = ref('Success')
const lastBackupTime = ref('2 hours ago')

// ── Resources ──────────────────────────────────────────────
const storage = ref({ used: 2.4, total: 10 })
const memoryPercent = ref(68)
const storagePercent = computed(() => (storage.value.used / storage.value.total) * 100)
...
// ── Optimization ───────────────────────────────────────────
const showOptimizationAlert = ref(true)
const lastOptimization = ref('7 days ago')
...
const backupHistory = ref([
  { id: 1, date: 'May 15, 2026 10:30 AM', type: 'Automatic', size: '24.3 MB', status: 'Success' },
  { id: 2, date: 'May 14, 2026 10:30 AM', type: 'Automatic', size: '23.8 MB', status: 'Success' },
  { id: 3, date: 'May 13, 2026 10:30 AM', type: 'Manual',    size: '23.1 MB', status: 'Success' },
  { id: 4, date: 'May 12, 2026 10:30 AM', type: 'Automatic', size: '22.9 MB', status: 'Failed'  },
])
...
// ── Tasks ──────────────────────────────────────────────────
const tasks = ref([
  { id: 1, name: 'Clear Cache',            description: 'Clear system cache to improve performance',         lastRun: '3 days ago',  running: false },
  { id: 2, name: 'Optimize Database',      description: 'Run VACUUM and ANALYZE on all tables',             lastRun: '7 days ago',  running: false },
  { id: 3, name: 'Clean Old Logs',         description: 'Remove log entries older than 30 days',            lastRun: '14 days ago', running: false },
  { id: 4, name: 'Check Expired Products', description: 'Scan inventory for products past expiration date', lastRun: 'Today',       running: false },
])
...
const fetchUsers = async () => {
  const { data } = await supabase.from('users').select('id, username, role, branch')
  if (data) {
    totalUsers.value = data.length
    activeUsers.value = Math.floor(data.length * 0.4)
    userSessions.value = data.slice(0, 5).map(u => ({
      id: u.id,
      username: u.username,
      role: u.role,
      branch: u.branch,
...
const fetchFailedLogins = async () => {
  // [Inference] Using a simple count from users table as proxy since no audit log table exists
...
const runBackup = async () => {
  ...
  backupHistory.value.unshift({ id: Date.now(), date: now, type: 'Manual', size: '24.5 MB', status: 'Success' })
...
const runTask = async (task) => {
  ...
  showToast(`${task.name} completed successfully.`, 'success')
}
...
.page-sub   { font-size: .82rem; color: #6b6b6b; }
...
.stat-label { font-size: 13px; color: #6b6b6b; font-weight: 500; }
.stat-icon  { color: #8B4513; }
.stat-icon.green { color: #28a745; }
.stat-value { font-size: 26px; font-weight: 700; color: #8B4513; margin-bottom: 4px; }
.stat-value.green { color: #28a745; }
.stat-sub   { font-size: 12px; color: #8B4513; }
...
.tab-btn {
  ...
  padding: .7rem 1.1rem; font-size: .84rem; font-weight: 600;
  color: #6b6b6b; border-bottom: 2px solid transparent;
  margin-bottom: -1px; cursor: pointer;
  transition: color .15s, border-color .15s;
}
.tab-btn:hover  { color: #532f15; }
.tab-btn.active { color: #532f15; border-bottom-color: #532f15; background: #fff8f4; border-radius: 8px 8px 0 0; }
...
.btn-optimize {
  ...
  background: #532f15; color: #fff; border: none;
  padding: 6px 14px; border-radius: 6px; font-size: 13px;
  font-weight: 600; cursor: pointer; transition: background .15s;
  display: inline-flex; align-items: center;
}
.btn-optimize:hover:not(:disabled) { background: #4E342E; }
.btn-optimize:disabled { opacity: .65; cursor: not-allowed; }

/* ── Backup ───────────────────────────────────────────────── */
.backup-actions { margin-bottom: 8px; }
.history-title  { font-size: 13px; font-weight: 700; color: #1a1a1a; margin-bottom: 12px; }

/* ── Tables ───────────────────────────────────────────────── */
.history-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.history-table th {
  background: #f8f9fa; color: #6b6b6b; font-weight: 700; font-size: 12px;
  padding: 10px 14px; text-align: left;
  border-top: 1px solid #e9ecef; border-bottom: 1px solid #e9ecef;
}
.history-table td {
  padding: 11px 14px; border-bottom: 1px solid #f5f5f5; color: #343a40;
}
...
.setting-row {
  ...
  padding: 16px 0; border-bottom: 1px solid #f0ebe8;
}
.setting-row:last-child { border-bottom: none; }
.setting-label { font-size: 14px; font-weight: 600; color: #1a1a1a; }
.setting-sub   { font-size: 12px; color: #6b6b6b; margin-top: 2px; }
.setting-input {
  width: 80px; padding: 6px 10px; border: 1px solid #e9ecef;
  border-radius: 6px; font-size: 14px; text-align: center; outline: none;
}
.setting-input:focus { border-color: #532f15; }
.setting-select {
  padding: 6px 10px; border: 1px solid #e9ecef;
  border-radius: 6px; font-size: 14px; outline: none; cursor: pointer;
}
.setting-select:focus { border-color: #532f15; }
...
.toggle input:checked + .toggle-slider { background: #532f15; }
...
.task-row {
  display: flex; justify-content: space-between; align-items: center;
  padding: 16px 0; border-bottom: 1px solid #f0ebe8; gap: 16px;
}
...
.btn-run {
  background: #fff; border: 1px solid #e9ecef; color: #532f15;
