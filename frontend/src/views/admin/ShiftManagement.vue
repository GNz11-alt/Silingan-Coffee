<template>
  <div class="schedule-page">
                  ...
                  <div class="avail-card-info">
                    ...
                    <div class="avail-meta">
                      ...
                      <span v-if="inq.branchId" class="ms-1">
                        <span
                          class="branch-dot"
                          :style="{ background: branchColor(inq.branchId) }"
                        ></span>
                  ...
                  <div class="avail-card-info">
                    ...
                    <div class="avail-meta">
                      ...
                      <span v-if="req.branchId" class="ms-1">
                        <span
                          class="branch-dot"
                          :style="{ background: branchColor(req.branchId) }"
                        ></span>
    ...
    <!-- ΓöÇΓöÇ All modals unchanged ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ -->
    <!-- CREATE / EDIT MODAL -->
    <Teleport to="body">
          ...
          <div class="modal-panel-body">
            ...
            <div class="row g-3">
              <div class="col-12">
                ...
                <div v-if="form.branchLocked" class="form-control-static">{{ branchName(form.branchId) }}</div>
                <select
                  v-else
              ...
              <div class="col-12 employee-search-wrap">
                ...
                <input
                  ...
                  @input="
                    ...
                    employeeHighlightIndex = filteredEmployeeList.length > 0
                      ? 0
                      : -1;
                  "
                  @keydown.up.prevent="
      ...
      <div v-if="showDayDetail" class="modal-overlay">
        ...
        >
          ...
          <div class="modal-panel-body modal-panel-body--day-detail">
            ...
            <template v-else>
              <div class="day-detail-empty-grid">
                <div class="dtd-grid-scroll">
                  <table class="dtd-grid">
                    <thead>
                      <tr>
                      ...
                      <tr v-for="role in roles" :key="role">
                        ...
                        >
                          <span
                            ...
                            class="dtd-cell-empty"
                            title="Add shift"
                            @click.stop="openCreateFromGrid(b.id, role)"
                          >
                            <span class="dtd-cell-empty-icon">+</span>
                          </span>
                          <span v-else class="dtd-cell-empty-past">&mdash;</span>
                ...
                <div class="day-detail-empty-hint" v-if="!isPastDate(showDayDetail.dateStr)">
                  <i class="bi bi-info-circle me-1"></i> Click any cell to schedule a staff member.
                </div>
                <div class="day-detail-empty-hint" v-else>
                  <i class="bi bi-lock-fill me-1"></i> No shifts were scheduled for this day.
                </div>
              </div>
            </template>
          </div>
        ...
        <div class="modal-panel" style="max-width: 620px; position: relative; z-index: 1" @click.stop>
          <div class="modal-panel-header">
            <div>
              ...
              <p class="text-muted small mb-0 mt-1">Set hours and shift periods per branch</p>
            </div>
          ...
          <div class="modal-panel-body" style="max-height: 70vh; overflow-y: auto">
            ...
            <template v-if="showOhTab === 'hours'">
              <div class="mb-3">
                <label class="form-label-sm">Branch</label>
                <select v-model="ohBranchId" class="form-select fc-brand" @change="onOhBranchChange">
                  <option v-for="b in branches" :key="b.id" :value="b.id">{{ b.name }}</option>
                </select>
              </div>
              <table class="oh-table" v-if="ohBranchId">
                <thead>
                  <tr>
                    <th>Day</th>
                    <th>Open</th>
            ...
            <template v-if="showOhTab === 'periods'">
              <div class="mb-2 d-flex justify-content-between align-items-center">
                <small class="text-muted">Define shift periods for the selected branch</small>
          ...
          <div class="modal-panel-footer d-flex justify-content-end gap-2">
            ...
            <button
              class="btn btn-primary-brand"
              @click="showOhTab === 'hours' ? saveOperatingHoursAction() : savePeriods(ohBranchId)"
...
// ΓöÇΓöÇ Operating Hours ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const showOperatingHoursModal = ref(false);
const ohBranchId = ref(null);
const ohDays = ref([]);
...
const openOperatingHours = async () => {
  ohBranchId.value = branches.value[0]?.id || null;
  ohDays.value = [];
  showOperatingHoursModal.value = true;
  await nextTick();
  if (ohBranchId.value) await loadOhDays();
};

const loadOhDays = async () => {
  if (!ohBranchId.value) return;
  const days = await fetchOperatingHours(ohBranchId.value);
  ohDays.value = days || [];
};

const onOhBranchChange = async () => {
  await loadOhDays();
};
...
const saveOperatingHoursAction = async () => {
  ohSaving.value = true;
  try {
    const success = await saveOperatingHours(ohBranchId.value, ohDays.value);
...
// ΓöÇΓöÇ Shift Periods ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Staff ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Password Reset Requests ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const fetchResetRequests = async (force = false) => {
  ...
  resetRequests.value = (data || []).map((r) => {
    const emp = employeeList.value.find((e) => e.id === r.employee_id);
    return {
      id: r.id,
...
// ΓöÇΓöÇ Employee search / dropdown ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const filteredEmployeeList = computed(() => {
  let list = employeeList.value.filter((e) => e.status === "Active");
...
// ΓöÇΓöÇ Staff CRUD ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const dayDetailBranches = computed(() => {
  ...
  for (const s of showDayDetail.value.shifts) {
    if (!seen.has(s.branchId)) {
      ...
      list.push({
        id: s.branchId,
        name: branchName(s.branchId),
        color: branchColor(s.branchId),
      });
    }
  }
...
// For empty days (no shifts), show all branches in the grid
const emptyDayBranches = computed(() => {
...
const onBranchChanged = async () => {
  if (form.value.branchLocked) return;
...
const openDayDetail = (day) => {
  ...
  nextTick(async () => {
    // Fetch periods for all known branches so tabs (Morning/Afternoon/Evening) always show
    for (const b of branches.value) {
      if (!(b.id in periodsByBranch.value) || !periodsByBranch.value[b.id]?.length) {
        await fetchPeriods(b.id);
      }
    }
    // Also fetch for any branch found in shifts (in case not in branches list yet)
...
const changeShift = () => {
  ...
  // focus the shift date input on next tick
  nextTick(() => {
...
const updateAvailStatus = async (avail, status) => {
  ...
  try {
    ...
    if (status === "Confirmed") {
      ...
      const availBranchId = emp?.BranchAssigned || null;
      if (availBranchId) {
        const dayOfWeek = new Date(avail.availableDate + "T12:00:00").getDay();
        if (!hoursByBranch.value[availBranchId]) {
          await fetchOperatingHours(availBranchId);
        }
        const availOh = getHoursForDay(availBranchId, dayOfWeek);
        if (availOh) {
          if (!availOh.isOpen) {
      ...
      if (availBranchId && avail.startTime) {
        const periods = periodsByBranch.value[availBranchId];
        if (periods?.length) {
          const match = periods.find(p =>
...
/* ΓöÇΓöÇ CALENDAR VIEW ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.calendar-container {
  background: #fff;
  border: 1px solid var(--border);
  border-radius: 10px;
...
/* Empty-day grid (shows all branches/roles with + buttons) */
.day-detail-empty-grid {
  display: flex;
  flex-direction: column;
  gap: 0.75rem;
}
.day-detail-empty-hint {
  font-size: 0.78rem;
  color: #9ca3af;
  text-align: center;
  padding: 0.5rem 0;
...
/* ΓöÇΓöÇ Staff tab ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.section-title {
  font-size: 1rem;
  font-weight: 700;
  color: #1a1a1a;
...
/* ΓöÇΓöÇ SPLIT LAYOUT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.split-layout {
  display: flex;
  gap: 1.25rem;
...
/* ΓöÇΓöÇ OPERATING HOURS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.oh-table {
  width: 100%;
  border-collapse: collapse;
}
...
/* ΓöÇΓöÇ OH TAB BAR ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.oh-tab-bar {
  display: flex;
  gap: 0;
  border-bottom: 1px solid #f0ebe8;
...
/* ΓöÇΓöÇ PERIOD COLUMNS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.dtd-th-period {
  min-width: 120px;
  font-weight: 600;
  font-size: 0.72rem;
...
/* ΓöÇΓöÇ Modal / Panel ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.modal-overlay {
  position: fixed;
  inset: 0;
...
/* ΓöÇΓöÇ Employee dropdown ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇ Badges ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.badge-status {
  display: inline-block;
  font-size: 0.68rem;
  font-weight: 700;
...
/* ΓöÇΓöÇ Shift Detail ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇ Toast ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
.toast-wrap {
  position: fixed;
  bottom: 1.5rem;
  right: 1.5rem;
