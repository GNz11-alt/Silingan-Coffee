...
export default {
  ...
  methods: {
    ...
    async loadSavedReports() {
      ...
      // resolve employee names for generatedby
      const empIds = [...new Set(rows.map(r => r.generatedby).filter(Boolean))];
      if (empIds.length) {
        ...
        rows.forEach(r => {
          if (r.generatedby && empMap[r.generatedby]) {
            r.generator = empMap[r.generatedby];
          }
        });
      }
    ...
    async generateReport() {
      ...
      try {
        ...
        // 5. Upload to cloud storage if opted in
        let filePath = null;
