...
const doDelete = async () => {
  ...
  if (deleteType.value === "batch") {
    ...
    // REPLACE the else block inside doDelete with:
  } else {
    // Soft-delete: set status to Archived on rawproduct
    // (keep rawproducttransaction intact for history)
    ...
    const { error } = await supabase
      .from("rawproduct")
      .update({
        status: "Archived", // add a status column if not present
        archivedDate: now,
