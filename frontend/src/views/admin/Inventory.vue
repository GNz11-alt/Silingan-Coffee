    ...
    <div class="page-header">
      <div class="ph-left">
        ...
        <p>
          ...
          <strong>{{ selectedBranchName || "All Branches" }}</strong>
        </p>
      </div>
  ...
  <div class="stat-card">
    ...
    <div v-if="expiringBatches.length > 0" class="fefo-alert">
      <div class="fefo-header">
        ...
        <p class="fefo-sub">
          These batches expire soonest. Use them before opening newer stock to
          minimize waste.
        </p>
      </div>
      <div class="fefo-list">
        <div
          v-for="batch in expiringBatches.slice(0, 5)"
          :key="batch.rawtransactionid"
          class="fefo-row"
        >
          <div class="fefo-item-info">
            <span class="fefo-name">{{ batch.rawproduct?.name ?? "—" }}</span>
            <span class="fefo-batch"
              >{{ batch.batchLabel }} · {{ batch.quantity }}
              {{ batch.rawproduct?.unit }}</span
            >
          </div>
          <div class="fefo-expiry-info">
            <span
              :class="[
                'fefo-days',
                getDaysUntilExpiry(batch.expirationdate) < 0
                  ? 'expired'
                  : getDaysUntilExpiry(batch.expirationdate) <= 3
                    ? 'critical'
                    : 'warning',
              ]"
            >
            ...
            <span class="fefo-date">{{
              formatDate(batch.expirationdate)
            }}</span>
          </div>
          <button
            v-if="getDaysUntilExpiry(batch.expirationdate) < 0"
            class="expire-btn"
            @click="markBatchExpired(batch)"
          >
            Mark Used/Discard
          </button>
        </div>
    ...
    <div v-if="eoqItems.length > 0" class="eoq-panel">
      ...
      <div class="eoq-list">
        <div
          v-for="item in eoqItems.slice(0, 4)"
        ...
        >
          <div class="eoq-item-info">
            ...
            <span class="eoq-stock">
              ...
              <span v-if="item._dailyUsage > 0" class="usage-hint"
                >· ~{{ item._dailyUsage.toFixed(2) }} {{ item.unit }}/day from
                sales</span
              >
            </span>
          </div>
          <div class="eoq-suggestion">
            ...
            <span class="eoq-qty"
              >{{ calculateEOQ(item) }} {{ item.unit }}</span
            >
          </div>
          <button class="eoq-restock-btn" @click="openRestockWithEOQ(item)">
            Add Stock →
          </button>
        </div>
      </div>
    ...
    <div class="tabs-row">
      ...
      <button
        :class="['tab-btn', activeTab === 'batches' ? 'active' : '']"
        @click="handleBatchTab"
      >
        <Layers :size="14" /> Batch Tracker
        <span v-if="expiringBatches.length > 0" class="tab-alert-dot">{{
    ...
    <div v-if="activeTab === 'items'" class="inventory-section">
      <div class="section-header">
        ...
        <div class="header-actions">
          ...
          <!-- ② NEW: Batch Restock button -->
          <button
            class="btn-batch-outline"
            @click="showBatchRestockModal = true"
          >
            <ClipboardList :size="13" /> Batch Restock
          </button>
          <!-- ③ NEW: Print Reorder Form button (only shown when low/out stock exists) -->
          <button
            v-if="eoqItems.length > 0"
        ...
        <table class="inventory-table">
          ...
          <tbody>
            <tr
              ...
              :key="item.rawproductid"
              :class="{ 'row-alert': getStatus(item) !== 'good' }"
            >
              <td>
                ...
                <span class="sku-label"
                  >SKU-{{ String(item.rawproductid).padStart(4, "0") }}</span
                >
              </td>
                    ...
                    <div
                      ...
                      :style="{
                        width:
                          Math.min(
                            100,
                            (item.stockquantity / (item.reorderlevel * 3)) *
                              100,
                          ) + '%',
                      }"
              ...
              <td>
                <span v-if="item.reorderlevel" class="eoq-chip"
                  >{{ calculateEOQ(item) }} {{ item.unit }}</span
                >
              ...
              <td>
                <div class="batch-summary" @click="openBatchDetail(item)">
                  <span class="batch-count"
                    >{{ getBatchCountForItem(item.rawproductid) }} batch{{
                      getBatchCountForItem(item.rawproductid) !== 1 ? "es" : ""
                    }}</span
                  >
                  <span
                    v-if="getExpiringBatchCount(item.rawproductid) > 0"
                    class="batch-expiring-warn"
                    >{{
                      getExpiringBatchCount(item.rawproductid)
                    }}
                    expiring</span
                  >
                </div>
              ...
              <td class="actions-cell">
                ...
                <button
                  class="icon-btn reduce"
                  @click="openReduceModalForItem(item)"
                  title="Reduce Stock"
                >
                  <MinusCircle :size="14" />
                </button>
      ...
      <div class="section-header">
        <div>
          ...
          <p class="section-subtitle">
            FEFO order — earliest expiring shown first ·
            {{ allBatches.length }} active batches
          </p>
        </div>
        <div class="fefo-legend">
          <span class="fl-item"
            ><span class="fl-dot expired"></span>Expired</span
          >
          <span class="fl-item"
            ><span class="fl-dot critical"></span>≤3 days</span
          >
          <span class="fl-item"
            ><span class="fl-dot warning"></span>≤7 days</span
          >
      ...
      <div v-else class="table-container">
        <table class="inventory-table">
          ...
          <tbody>
            ...
            >
              <td>
                <span class="batch-id-tag">{{ batch.batchLabel }}</span>
              </td>
              <td>
                <strong class="item-name-text">{{
                  batch.rawproduct?.name ?? "—"
                }}</strong>
              </td>
              <td>
              ...
              <td>
                <span
                  class="qty-val"
                  :class="getBatchExpiryClass(batch.expirationdate)"
                  >{{ batch.quantity }}</span
                >
              ...
              <td>
                <span
                  :class="[
                    'expiry-val',
                    getBatchExpiryClass(batch.expirationdate),
                  ]"
                >
                  {{
              ...
              <td>
                <span
                  ...
                  >{{ getDaysLabel(batch.expirationdate) }}</span
                >
              </td>
              <td>
                <span
                  ...
                  >{{ getFEFOLabel(batch.expirationdate) }}</span
                >
              </td>
              <td class="actions-cell">
                <button
      ...
      <div class="modal-content">
        <div class="modal-header">
          <div>
            <h2>Add New Item</h2>
            <p class="modal-sub">
              Register a new raw material — a unique SKU will be assigned
            </p>
          </div>
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input
              ...
              placeholder="e.g. Whole Milk, Arabica Beans, Paper Cups 16oz"
            />
            <span v-if="newItemErrors.name" class="field-error">{{
              newItemErrors.name
            }}</span>
          </div>
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <span v-if="newItemErrors.unit" class="field-error">{{
                newItemErrors.unit
              }}</span>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label
                >Reorder Point
                <span class="label-hint">— alert threshold</span></label
              >
              <input
                type="number"
                v-model.number="newItemForm.reorderlevel"
                min="0"
                placeholder="e.g. 500"
              />
            </div>
            <div class="form-group">
              <label
                ...
                <span class="label-hint"
                  >— days from order to arrival</span
                ></label
              >
              <input
                type="number"
                v-model.number="newItemForm.leadtimedays"
                min="1"
                placeholder="2"
              />
            </div>
          ...
          <div class="hasexpiry-toggle-row">
            ...
            />
            <label for="hasexpiry-toggle"
              >This item has an expiration date</label
            >
            <span class="hasexpiry-hint">{{
              newItemForm.hasexpiry
                ? "e.g. milk, syrups, dairy"
                : "e.g. cups, straws, napkins"
            }}</span>
          </div>
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input
                type="number"
                v-model.number="newItemForm.stockquantity"
                min="0"
                placeholder="0"
              />
            </div>
            <div class="form-group" v-if="newItemForm.hasexpiry">
              <label
                >Expiration Date
                <span class="label-hint" style="color: #dc2626"
                  >* required</span
                ></label
              >
              <input type="date" v-model="newItemForm.expirationdate" />
              <span v-if="newItemErrors.expirationdate" class="field-error">{{
                newItemErrors.expirationdate
              }}</span>
            </div>
          ...
          <div class="info-box">
            <Info :size="13" />
            <span
              >A unique <strong>SKU</strong> will be auto-assigned. Each time
              you add stock, a new <strong>batch</strong> is created with its
              own expiry date — enabling FEFO tracking. EOQ is calculated from
              your actual menu sales.</span
            >
          </div>
          <div class="modal-actions">
            <button class="btn-secondary" @click="closeNewItemModal">
              Cancel
            </button>
            <button
              class="btn-primary"
      ...
      <div class="modal-content restock-modal">
        <div class="modal-header">
          <div>
            <h2>Add Stock</h2>
            <p class="modal-sub">
              Each delivery creates a new tracked batch with its own expiry date
            </p>
          </div>
        ...
        <div class="modal-body">
          <div v-if="!restockTarget && !restockSelectedId" class="step-block">
            <p class="step-label">
              <span class="step-num">1</span> Select an item to restock
            </p>
            <div class="item-picker-grid">
              <button
                v-for="i in allRawItems"
                :key="i.rawproductid"
                class="item-picker-card"
                :class="{
                  selected: restockSelectedId === i.rawproductid,
                  alert: getStatus(i) !== 'good',
                }"
                @click="restockSelectedId = i.rawproductid"
              >
                <div class="ipc-top">
                  <span class="ipc-sku"
                    >SKU-{{ String(i.rawproductid).padStart(4, "0") }}</span
                  >
                  <span :class="['ipc-status', getStatus(i)]">{{
                    getStatusText(i)
                  }}</span>
                </div>
                <span class="ipc-name">{{ i.name }}</span>
                <span class="ipc-stock"
                  >{{ i.stockquantity }} {{ i.unit }} in stock</span
                >
              </button>
            </div>
            <span v-if="restockErrors.item" class="field-error">{{
              restockErrors.item
            }}</span>
          </div>
          <div v-else>
            <div class="restock-target-card" v-if="restockPreviewItem">
              <div class="rtc-top">
                <div class="rtc-name">
                  <Package :size="13" /><strong>{{
                    restockPreviewItem.name
                  }}</strong>
                </div>
                <span class="rtc-sku"
                  >SKU-{{
                    String(restockPreviewItem.rawproductid).padStart(4, "0")
                  }}</span
                >
              </div>
              <div class="rtc-stat-grid">
                <div class="rtc-stat">
                  ...
                  <span :class="['rtcs-val', getStatus(restockPreviewItem)]"
                    >{{ restockPreviewItem.stockquantity }}
                    {{ restockPreviewItem.unit }}</span
                  >
                </div>
                <div class="rtc-stat" v-if="restockPreviewItem.reorderlevel">
                  <span class="rtcs-label">Reorder Point</span>
                  <span class="rtcs-val neutral"
                    >{{ restockPreviewItem.reorderlevel }}
                    {{ restockPreviewItem.unit }}</span
                  >
                </div>
                <div class="rtc-stat" v-if="restockPreviewItem.reorderlevel">
                  <span class="rtcs-label">EOQ Suggestion</span>
                  <span class="rtcs-val blue"
                    >{{ calculateEOQ(restockPreviewItem) }}
                    {{ restockPreviewItem.unit }}</span
                  >
                </div>
                <div class="rtc-stat" v-if="restockPreviewItem._dailyUsage > 0">
                  ...
                  <span class="rtcs-val neutral"
                    >~{{ restockPreviewItem._dailyUsage.toFixed(2) }}
                    {{ restockPreviewItem.unit }}/day</span
                  >
                </div>
              </div>
              <button
            ...
            <div v-if="!selectedBranchId" class="branch-required-block">
              <label class="bfg-label"
                >Destination Branch <span class="label-hint"></span
              ></label>
              ...
              <span v-if="restockErrors.branch" class="field-error">{{
                restockErrors.branch
              }}</span>
            </div>
            <div v-else class="branch-assigned-note">
              <span
                >Stock will be added to
                <strong>{{ selectedBranchName }}</strong></span
              >
            </div>
            ...
            <div class="batch-form-grid">
              <div class="bfg-card">
                ...
                <div class="bfg-qty-row">
                  <input
                    ...
                    class="bfg-qty-input"
                  />
                  <span class="bfg-unit-badge">{{
                    restockPreviewItem?.unit
                  }}</span>
                </div>
                <button
                  ...
                  class="eoq-fill-btn"
                  @click="
                    restockForm.quantity = calculateEOQ(restockPreviewItem)
                  "
                >
                  <TrendingUp :size="12" /> Fill EOQ ({{
                    calculateEOQ(restockPreviewItem)
                  }})
                </button>
                <span v-if="restockErrors.quantity" class="field-error">{{
                  restockErrors.quantity
                }}</span>
              </div>
              <div
                class="bfg-card"
                v-if="restockPreviewItem?.hasexpiry !== false"
              >
                <label class="bfg-label"
                  >Expiry Date
                  <span class="label-hint" style="color: #dc2626"
                ...
                <input
                  ...
                  :style="
                    restockErrors.expiry
                      ? 'border-color:#dc2626;box-shadow:0 0 0 3px rgba(220,38,38,.08)'
                      : ''
                  "
                />
                <span v-if="restockErrors.expiry" class="field-error">{{
                  restockErrors.expiry
                }}</span>
              </div>
              <div class="bfg-card" v-else>
                ...
                <div class="no-expiry-note" style="margin-top: 8px">
                  <span
                    ><strong>{{ restockPreviewItem?.name }}</strong> is
                    non-perishable — no expiry date needed.</span
                  >
                </div>
              </div>
            </div>
            <div
              class="restock-preview"
              v-if="restockPreviewItem && restockForm.quantity > 0"
            >
              <div class="rp-row">
              ...
              <div class="rp-row total">
                <span>New total</span
                ><span
                  >{{ restockPreviewItem.stockquantity + restockForm.quantity }}
                  {{ restockPreviewItem.unit }}</span
                >
              </div>
              <div class="rp-row">
            ...
            <div class="fefo-note">
              <Layers :size="12" />
              <span
                >A new batch will be logged under
                <strong>{{ restockPreviewItem?.name }}</strong> with its own
                expiry date. Staff will be reminded to use earliest-expiring
                batches first.</span
              >
            </div>
          </div>
          <div class="modal-actions">
            <button class="btn-secondary" @click="closeRestockModal">
              Cancel
            </button>
            <button
              class="btn-primary"
              :disabled="
                savingRestock || (!restockTarget && !restockSelectedId)
              "
              @click="saveRestock"
            >
    ...
    <!-- ══ BATCH DETAIL MODAL ══ -->
    <div
      v-if="showBatchDetail"
      class="modal"
      @click.self="showBatchDetail = false"
    >
      <div class="modal-content" style="max-width: 680px">
        <div class="modal-header">
          <div>
            ...
            <p class="modal-sub">
              SKU-{{ String(batchDetailItem?.rawproductid).padStart(4, "0") }} ·
              each row = one delivery/restock · FEFO order
            </p>
          </div>
        ...
        <div class="modal-body" style="padding: 0">
          ...
          <div v-if="itemBatches.length > 0" class="batch-total-row">
            <span
              >Total across {{ itemBatches.length }} batch{{
                itemBatches.length !== 1 ? "es" : ""
              }}:</span
            >
            <strong
              >{{ itemBatches.reduce((s, b) => s + (b.quantity || 0), 0) }}
              {{ batchDetailItem?.unit }}</strong
            >
          </div>
        </div>
      </div>
    ...
    <Teleport to="body">
      ...
      >
        <div class="modal-content" style="max-width: 420px">
          <div class="modal-header">
            <h2>
              {{ deleteType === "batch" ? "Remove Batch" : "Remove Item" }}
            </h2>
          ...
          <div class="modal-body">
            <p style="font-size: 14px; margin-bottom: 16px">
              <span v-if="deleteType === 'batch'">
                ...
                {{ deleteTarget?.rawproduct?.unit ?? batchDetailItem?.unit }})?
                The stock total will decrease by this amount.
              </span>
              <span v-else>
                Remove <strong>{{ deleteTarget?.name }}</strong> and ALL its
      ...
      <div class="modal-content">
        <div class="modal-header">
          <div>
            ...
            <p class="modal-sub">
              Update details for
              <strong>{{ editItemForm.name || "this item" }}</strong>
            </p>
          </div>
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            />
            <span v-if="editItemErrors.name" class="field-error">{{
              editItemErrors.name
            }}</span>
          </div>
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <span v-if="editItemErrors.unit" class="field-error">{{
                editItemErrors.unit
              }}</span>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              ...
              <input
                type="number"
                v-model.number="editItemForm.reorderlevel"
                min="0"
                placeholder="e.g. 500"
              />
            </div>
            <div class="form-group">
              <label
                >Lead Time (days)
                <span class="label-hint">— days to arrival</span></label
              >
              <input
                type="number"
                v-model.number="editItemForm.leadtimedays"
                min="1"
                placeholder="2"
              />
            </div>
          ...
          <div class="hasexpiry-toggle-row">
            <input
              type="checkbox"
              id="edit-hasexpiry-toggle"
              v-model="editItemForm.hasexpiry"
            />
            <label for="edit-hasexpiry-toggle"
              >This item has an expiration date</label
            >
            <span class="hasexpiry-hint">{{
              editItemForm.hasexpiry
                ? "e.g. milk, syrups, dairy"
                : "e.g. cups, straws, napkins"
            }}</span>
          </div>
    ...
    <PrintReorderForm
      :show="showPrintModal"
      :items="allRawItems"
      :branch-name="selectedBranchName || 'All Branches'"
...
// ── NEW modal visibility flags ───────────────────────────────────────────────
const showBatchRestockModal = ref(false);
const showReduceModal = ref(false);
const showPrintModal = ref(false);
const reducePreselectedItem = ref(null); // for per-row Reduce button
...
const onReduceDone = async ({ item, qty, reason }) => {
  showToast(
    `✓ Reduced ${qty} ${item.unit} of ${item.name} (${reason}).`,
    "success",
  );
...
const onBatchRestockDone = async ({ count }) => {
  showToast(
    `✓ Batch restock complete — ${count} item type${count !== 1 ? "s" : ""} updated.`,
    "success",
  );
  await Promise.all([fetchBatches(), fetchRawMaterials()]);
};
...
const buildBatchLabels = (batches) => {
  ...
  const sorted = [...batches].sort(
    (a, b) => a.rawtransactionid - b.rawtransactionid,
  );
...
const attachLabels = (batches, labelMap) =>
  batches.map((b) => ({
    ...b,
    batchLabel: labelMap[b.rawtransactionid] ?? `#${b.rawtransactionid}`,
  }));
...
const calculateEOQ = (item) => {
  ...
  const dailyUsage =
    item._dailyUsage > 0 ? item._dailyUsage : item.reorderlevel / 14;
...
const fetchDailyUsageFromSales = async () => {
  ...
  for (const row of orderItems ?? []) {
    unitsSold[row.ProductId] =
      (unitsSold[row.ProductId] ?? 0) + (row.Quantity ?? 0);
  }
  ...
  for (const recipe of recipes ?? []) {
    ...
    usageMap[recipe.rawproductid] =
      (usageMap[recipe.rawproductid] ?? 0) +
      sold * (recipe.quantityneeded ?? 0);
  }
...
const selectedBranchName = computed(
  () =>
    branches.value.find((b) => b.BranchId === selectedBranchId.value)
      ?.BranchName ?? "",
);
const getBranchName = (branchId) => {
  if (!branchId) return "—";
  return (
    branches.value.find((b) => b.BranchId === branchId)?.BranchName ??
    `Branch ${branchId}`
  );
};
...
const filteredBatches = computed(() => {
  ...
  if (batchFilterExpiry.value)
    list = list.filter(
      (b) => getBatchExpiryClass(b.expirationdate) === batchFilterExpiry.value,
    );
  return list;
});

const expiringBatches = computed(() =>
  sortedBatches.value.filter(
    (b) => b.expirationdate && getDaysUntilExpiry(b.expirationdate) <= 7,
  ),
);
...
const restockPreviewItem = computed(() => {
  ...
  if (restockSelectedId.value)
    return (
      allRawItems.value.find(
        (i) => i.rawproductid === restockSelectedId.value,
      ) ?? null
    );
  return null;
});
...
const getExpiringBatchCount = (id) => {
  let batches = allBatches.value.filter(
    (b) =>
      b.rawproductid === id &&
      b.expirationdate &&
      getDaysUntilExpiry(b.expirationdate) <= 7,
  );
  if (selectedBranchId.value)
    batches = batches.filter((b) => b.branchid === selectedBranchId.value);
  return batches.length;
  ...
  items.forEach((item) => {
    ...
    item.stockquantity = Math.max(
      ...
      batches.reduce((sum, b) => {
        const qty =
          b.transactiontype === "in" ? (b.quantity ?? 0) : -(b.quantity ?? 0);
        return sum + qty;
      }, 0),
    );
  });
...
const fetchBatches = async () => {
  ...
  const { data, error } = await supabase
    ...
    .select(
      `rawtransactionid, rawproductid, branchid, transactiontype, quantity, expirationdate, createdat, rawproduct ( name, unit, category )`,
    )
    .gt("quantity", 0)
...
const saveNewItem = async () => {
  ...
  if (
    newItemForm.value.hasexpiry &&
    newItemForm.value.stockquantity > 0 &&
    !newItemForm.value.expirationdate
  )
  ...
  showToast(
    `"${newItem.name}" added — SKU-${String(newItem.rawproductid).padStart(4, "0")}`,
    "success",
  );
  await Promise.all([fetchBatches(), fetchRawMaterials()]);
  closeNewItemModal();
...
const saveRestock = async () => {
  ...
  if (!restockPreviewItem.value) {
    ...
    if (
      restockPreviewItem.value.hasexpiry !== false &&
      !restockForm.value.expirationdate
    )
  ...
  if (
    ...
    batchDetailItem.value?.rawproductid === item.rawproductid
  ) {
...
const openBatchDetail = async (item) => {
  ...
  let query = supabase
    .from("rawproducttransaction")
    .select(
      "rawtransactionid, rawproductid, branchid, quantity, expirationdate, createdat",
    )
    .eq("rawproductid", item.rawproductid)
  ...
  const labelled = rows.map((b, i) => ({
    ...
    batchLabel:
      i === 0 ? `B${item.rawproductid}` : `B${item.rawproductid}-${i + 1}`,
  }));
...
const doDelete = async () => {
  ...
  if (deleteType.value === "batch") {
    ...
    const newQty = Math.max(
      0,
      (fresh?.stockquantity ?? 0) - (batch.quantity ?? 0),
    );
    await supabase
      .from("rawproduct")
...
const saveEditItem = async () => {
  ...
  if (error) {
    ...
    showToast(
      `✓ "${editItemForm.value.name}" updated successfully.`,
      "success",
    );
    await fetchRawMaterials();
    closeEditModal();
...
onMounted(async () => {
  ...
  if (editId) {
    const item = allRawItems.value.find(
      (r) => String(r.rawproductid) === editId,
    );
...
.inventory-table tr.row-alert td {
  background: #fffbf0;
}
.inventory-table tr.batch-row.expired td {
  background: #fff5f5;
}
.inventory-table tr.batch-row.critical td {
  background: #fff7ed;
}
.inventory-table tr.batch-row.warning td {
  background: #fffbeb;
}
...
.batch-summary:hover .batch-count {
...
.fefo-status-badge.ok {
  background: #e8f5e9;
  color: #2e7d32;
}
.fefo-status-badge.warning {
  background: #fff3e0;
  color: #d97706;
}
.fefo-status-badge.critical {
  background: #fef0e8;
  color: #dc2626;
}
.fefo-status-badge.expired {
  background: #ffebee;
  color: #dc2626;
}
...
.icon-btn.restock:hover {
  background: #e8f5e9;
}
.icon-btn.reduce {
  color: #dc2626;
}
.icon-btn.reduce:hover {
  background: #ffebee;
}
.icon-btn.batches {
  color: #7c3aed;
}
.icon-btn.batches:hover {
  background: #f3e8ff;
}
.icon-btn.delete {
  color: #dc3545;
...
@media (max-width: 768px) {
  ...
  /* Section header — stack, buttons wrap in rows of 2 */
  .section-header {
    flex-direction: column;
    align-items: flex-start;
    gap: 10px;
