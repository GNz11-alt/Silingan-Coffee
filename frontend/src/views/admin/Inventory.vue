      ...
      <div class="modal-content" style="max-width: 680px">
        ...
        <div class="modal-body" style="padding: 0">
          ...
          <table v-else class="inventory-table">
            <thead>
              <tr>
                <th>Sub-Batch</th><th>Branch</th><th>Qty (this batch)</th>
...
const sortedBatches = computed(() => {
  ...
  let list = labelled;
  if (selectedBranchId.value) list = list.filter((b) => b.branchid === selectedBranchId.value);
...
const filteredBatches = computed(() => {
  ...
  if (batchSearch.value) {
    ...
    list = list.filter((b) =>
      b.rawproduct?.name?.toLowerCase().includes(q) || b.batchLabel?.toLowerCase().includes(q) || String(b.rawtransactionid).includes(q)
    );
  }
...
const onBranchChange = async () => { await fetchBatches(); await fetchRawMaterials(); };
...
const saveNewItem = async () => {
  ...
  const { data: newItem, error: itemErr } = await supabase
    .from("rawproduct")
    .insert([{ name: newItemForm.value.name.trim(), category: newItemForm.value.category || null, unit: newItemForm.value.unit, stockquantity: newItemForm.value.stockquantity || 0, reorderlevel: newItemForm.value.reorderlevel || null, leadtimedays: newItemForm.value.leadtimedays || 2, expirationdate: newItemForm.value.expirationdate || null, hasexpiry: newItemForm.value.hasexpiry }])
    .select().single();
...
const openRestockModal = (item) => {
  restockErrors.value = {};
  restockForm.value = { quantity: 0, expirationdate: "", branchId: selectedBranchId.value ?? null };
...
const saveRestock = async () => {
  const e = {};
  if (!restockPreviewItem.value) { e.item = "Please select an item."; }
  else {
    if (!restockForm.value.quantity || restockForm.value.quantity <= 0) e.quantity = "Enter a valid quantity.";
    if (!selectedBranchId.value && !restockForm.value.branchId) e.branch = "Please select a destination branch.";
  ...
  if (selectedBranchId.value) {
    const { data: fresh } = await supabase.from("rawproduct").select("stockquantity").eq("rawproductid", item.rawproductid).single();
    actualTotal = (fresh?.stockquantity ?? 0) + restockForm.value.quantity;
  }

  const { error: upErr } = await supabase.from("rawproduct").update({ stockquantity: actualTotal }).eq("rawproductid", item.rawproductid);
  if (upErr) { showToast("Failed to update stock: " + upErr.message, "error"); savingRestock.value = false; return; }

  const { error: txErr } = await supabase.from("rawproducttransaction").insert([{
    rawproductid: item.rawproductid, branchid: targetBranchId,
    transactiontype: "in", quantity: restockForm.value.quantity,
    expirationdate: restockForm.value.expirationdate || null,
  }]);

  if (txErr) { showToast(`Stock updated but batch log failed: ${txErr.message}`, "error"); savingRestock.value = false; await Promise.all([fetchBatches(), fetchRawMaterials()]); closeRestockModal(); return; }
...
const doDelete = async () => {
  ...
  if (deleteType.value === "batch") {
    ...
    const { data: fresh } = await supabase.from("rawproduct").select("stockquantity").eq("rawproductid", batch.rawproductid).single();
    ...
    const { error } = await supabase
      .from("rawproduct")
      .update({ status: "Archived", archivedDate: now, archivedBy: currentUser })
