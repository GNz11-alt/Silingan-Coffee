<template>
  <div class="inventory-content">
    ...
    <div v-if="!assignedBranchId && !loadingBranch" class="no-branch-card">
      ...
      <p>
        Your manager account has not been assigned to a branch yet. Please
                  ...
                  <div class="batch-summary" @click="openBatchDetail(item)">
                    <span class="batch-count">
                      {{ getBatchCountForItem(item.rawproductid) }} batch{{
                        getBatchCountForItem(item.rawproductid) !== 1
                          ? "es"
                          : ""
                      }}
                    </span>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <div>
              <h2>Add New Item</h2>
              <p class="modal-sub">
                Will be added to <strong>{{ assignedBranchName }}</strong> — a
                unique SKU will be assigned
              </p>
            </div>
          ...
          <div class="modal-body">
            ...
            <div class="info-box">
              <Info :size="13" />
              <span
                >Stock will be added to <strong>{{ assignedBranchName }}</strong
                >. A unique <strong>SKU</strong> will be auto-assigned and FEFO
                tracking will be enabled per batch.</span
              >
            </div>
        ...
        <div class="modal-content restock-modal">
          <div class="modal-header">
            <div>
              <h2>Add Stock</h2>
              <p class="modal-sub">
                Adding to <strong>{{ assignedBranchName }}</strong> — each
                delivery creates a new tracked batch
              </p>
            </div>
            ...
            <div v-else>
              <div class="restock-target-card" v-if="restockPreviewItem">
                ...
                <div class="rtc-stat-grid">
                  ...
                  <div
                    class="rtc-stat"
                    v-if="restockPreviewItem._dailyUsage > 0"
                  >
              ...
              <div class="branch-assigned-note">
                <span
                  ...
                  <strong>{{ assignedBranchName }}</strong></span
                >
              </div>
        ...
        <div class="modal-content" style="max-width: 640px">
          <div class="modal-header">
            <div>
              ...
              <p class="modal-sub">
                SKU-{{
                  String(batchDetailItem?.rawproductid).padStart(4, "0")
                }}
                · {{ assignedBranchName }} · FEFO order
              </p>
            </div>
            <button class="close-btn" @click="showBatchDetail = false">
              ×
            </button>
          </div>
            ...
            <div class="modal-body">
              <p style="font-size: 14px; margin-bottom: 16px">
                <span v-if="deleteType === 'batch'">
                  Remove batch
                  <strong>{{ deleteTarget?.batchLabel }}</strong> ({{
                    deleteTarget?.quantity
                  }}
                  {{
                    deleteTarget?.rawproduct?.unit ?? batchDetailItem?.unit
                  }})? Stock total will decrease by this amount.
                </span>
                <span v-else>
...
const fetchDailyUsageFromSales = async () => {
  ...
  const { data: orderItems, error: oi_err } = await supabase
    .from("orderitem")
    .select(
      `ProductId, Quantity, orders!inner ( OrderId, CreatedAt, Status, BranchId )`,
    )
  ...
  if (oi_err) {
    console.error("fetchDailyUsage error:", oi_err);
    return {};
  }
  ...
  if (r_err) {
    console.error("fetchDailyUsage recipe error:", r_err);
    return {};
  }
  ...
  for (const recipe of recipes ?? []) {
    ...
    usageMap[recipe.rawproductid] =
      (usageMap[recipe.rawproductid] ?? 0) + totalUsed;
  }
...
const fetchRawMaterials = async () => {
  ...
  const itemsWithPresence = items.filter((item) => {
    const hasBatches = allBatches.value.some(
      (b) => b.rawproductid === item.rawproductid,
    );
...
const saveRestock = async () => {
  ...
  showToast(
    `✓ Added ${restockForm.value.quantity} ${item.unit} of ${item.name}.`,
    "success",
  );
  await Promise.all([fetchBatches(), fetchRawMaterials()]);
  if (
