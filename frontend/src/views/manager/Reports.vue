                  ...
                  <tr v-for="r in savedReports" :key="r.reportid" class="report-row">
                    ...
                    <td class="col-title">
                      <div class="report-title-cell">
                        ...
                        <div class="report-title-sub">Generated by {{ r.generator?.firstname || 'Manager' }}</div>
                      </div>
                    </td>
...
export default {
  ...
  data() {
    ...
    // Read manager's branch from localStorage — set during login
    const managerBranchId   = Number(localStorage.getItem('branchId'))   || null
    const managerBranchName = localStorage.getItem('branchName') || ''

    return {
      // Manager branch — locked, never changes
      managerBranchId,
      managerBranchName,
      ...
      genForm: {
        ...
        dateFrom: weekAgo,
        dateTo: today,
        branchId: managerBranchId, // always locked to manager's branch
        format: 'pdf',
      },
      genErrors: {},
  ...
  async mounted() {
    // If branchName wasn't in localStorage, try to resolve it from branchId
    if (!this.managerBranchName && this.managerBranchId) {
      await this.resolveBranchName()
    }
  ...
  methods: {
    ...
    async resolveBranchName() {
      const { data, error } = await getBranches()
      if (!error && data) {
        const branch = data.find(b => b.branchid === this.managerBranchId)
        if (branch) {
          this.managerBranchName = branch.branchname
    ...
    async loadInsights() {
      ...
      try {
        ...
        this.insightsData = generateInsights({
          ...
          branchComparison:  [], // not applicable for single-branch manager
    ...
    resetGenForm() {
      ...
      this.genForm  = {
        ...
        dateFrom: weekAgo,
        dateTo: today,
        branchId: this.managerBranchId, // keep branch locked
        format: 'pdf',
      }
      this.genErrors = {}
    },
