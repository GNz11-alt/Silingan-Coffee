<template>
  <div class="menu-module">
    ...
    <div class="stock-legend-bar">
      <span class="leg-item"><span class="leg-dot oos"></span>Out of Stock</span>
      <span class="leg-item"><span class="leg-dot low"></span>Low Stock</span>
      <span class="leg-item"><span class="leg-dot recipe-ok"></span>Recipe Set</span>
      <span class="leg-item"><span class="leg-dot no-recipe"></span>No Recipe</span>
    ...
    <div v-else v-for="(items, category) in filteredGrouped" :key="category" class="menu-section">
      <h3 class="section-title">
        ...
        <span v-if="items.some(i => getStockStatus(i) === 'oos')" class="section-oos-warn">
          <AlertTriangle :size="12" /> {{ items.filter(i => getStockStatus(i) === 'oos').length }} out of stock
        </span>
      </h3>
      <div class="menu-grid">
        <div
          ...
          v-for="item in items"
          :key="item.ProductId"
          :class="[`stock-${getStockStatus(item)}`, item._recipeCount === 0 ? 'no-recipe-card' : '']"
        >
          <!-- Stock status ribbon -->
          <div :class="['stock-ribbon', `ribbon-${getStockStatus(item)}`]">
            <span v-if="getStockStatus(item) === 'oos'"><XCircle :size="10" /> Out of Stock</span>
            <span v-else-if="getStockStatus(item) === 'low'"><AlertTriangle :size="10" /> Low Stock</span>
            <span v-else-if="item._recipeCount === 0"><BookOpen :size="10" /> No Recipe</span>
          ...
          <div class="card-body">
            ...
            <div v-if="item._recipeCount > 0 && item._stockDetail" class="stock-detail-row">
              <span v-for="shortage in item._stockDetail.slice(0,2)" :key="shortage.rawproductid" class="shortage-chip">
                <AlertTriangle :size="9" /> {{ shortage.name }}: {{ +shortage.available.toFixed(2) }}/{{ +shortage.needed.toFixed(2) }} {{ shortage.unit }}
              </span>
            </div>
          </div>
          ...
          <div class="recipe-status-area">
            ...
            >
              <ChefHat :size="13" /> Recipe ({{ item._recipeCount }} ingredient{{ item._recipeCount !== 1 ? 's' : '' }})
            </button>
          ...
          <div class="price-action-row">
            ...
            <div class="card-actions">
              <button class="icon-btn edit" @click="openEditModal(item)" title="Edit"><Edit2 :size="15" /></button>
              <button class="icon-btn delete" @click="deleteItem(item.ProductId)" title="Delete"><Trash2 :size="15" /></button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="modal-content">
        <header class="modal-hdr">
          <div>
            ...
            <p>{{ isEditing ? 'Updates will apply across all branches' : 'New item will be available to all branches' }}</p>
          </div>
        ...
        <div class="form-stack">
          ...
          <div class="field" v-if="getSizeType(form.Category) !== 'none'">
            ...
            <div class="sizes-container">
              <div v-for="size in getSizeLabels(form.Category)" :key="size" class="size-price-row">
                <span class="size-name">{{ size }}</span>
                <div class="size-price-input">
                  <span class="currency">₱</span>
                  <input type="number" v-model.number="form.sizePrices[size]" step="0.01" min="0" :placeholder="size + ' price'" required />
                </div>
              </div>
            </div>
          </div>
          <div class="field" v-if="getSizeType(form.Category) === 'none'">
            <label>Price (₱)</label>
            <input type="number" v-model.number="form.Price" step="0.01" min="0" placeholder="0.00" />
          </div>
        </div>
    ...
    <div v-if="showRecipeModal" class="modal-overlay" @click.self="closeRecipeModal">
      <div class="modal-content recipe-modal">
        ...
        <div v-else class="recipe-body">
          ...
          <div v-if="recipeStockPreview.length > 0" class="recipe-stock-preview">
            <div class="rsp-header"><AlertTriangle :size="14" /> Stock issues with current recipe:</div>
            <div v-for="s in recipeStockPreview" :key="s.rawproductid" class="rsp-row">
              <span class="rsp-name">{{ s.name }}</span>
              <span class="rsp-detail">{{ +s.available.toFixed(4) }} {{ s.unit }} available, need {{ +s.needed.toFixed(4) }} {{ s.unit }}</span>
            </div>
          </div>
          <div v-else-if="recipeRows.length > 0 && !loadingRecipe" class="recipe-stock-ok">
          ...
          <div class="ingredient-list">
            ...
            <div class="field-item">
              ...
              <!-- Quantity -->
              <div class="field-item">
                <input v-model.number="ing.quantityneeded" type="number" min="0" step="any" placeholder="0" />
              </div>
              ...
              <div class="field-item unit-field">
                <div class="select-wrap full">
                  <select v-model="ing.unit">
                    <option v-for="u in getCompatibleUnits(ing.rawproductid)" :key="u">{{ u }}</option>
                  </select>
                  <ChevronDown :size="12" class="sel-icon" />
                </div>
                <span v-if="ing.unit && ing.rawproductid && getInventoryUnit(ing.rawproductid) !== ing.unit" class="convert-hint">
                  → {{ formatConvertHint(ing.quantityneeded, ing.unit, getInventoryUnit(ing.rawproductid)) }}
                </span>
              </div>
      ...
      <div class="modal-content all-recipes-modal">
        ...
        <div v-else class="all-recipes-layout">
          <div class="recipe-sidebar">
            <button
              v-for="cat in allRecipeCategories" :key="cat"
              class="cat-tab"
          ...
          <div class="recipe-panel">
            ...
            <div class="recipe-panel-scroll">
              <div
                v-for="row in filteredRecipePanel" :key="row.ProductId"
                class="recipe-card"
              ...
              >
                ...
                <div v-if="expandedRecipe === row.ProductId" class="recipe-card-body">
                  <div v-for="r in row.recipes" :key="r.recipeid" class="ing-row">
                    ...
                    <span v-if="r.unit !== r.rawUnit && r.rawUnit" class="ing-inv-unit">(inv: {{ r.rawUnit }})</span>
                  </div>
                </div>
              </div>
...
import {
  Plus, Search as SearchIcon, ChevronDown, Edit2, Trash2, X,
  Coffee, ChefHat, BookOpen, AlertTriangle, CheckCircle, XCircle,
...
const CACHE_TTL = 5 * 60 * 1000 // 5 min (shorter so stock stays fresh)
...
const loadCache = (key) => {
  ...
  if (Date.now() - parsed.timestamp > CACHE_TTL) { sessionStorage.removeItem(key); return null }
  return parsed.data
}
...
function getInventoryUnit(rawproductid) {
  return rawProducts.value.find(r => r.rawproductid === rawproductid)?.unit ?? null
}
...
const form       = ref({ ProductName: '', Category: '', Price: null, sizePrices: {}, image_url: null })
...
function getStockStatus(item) {
  ...
  const shortages = checkStockForProduct(
    item.ProductId, 1, allRecipes.value, stockMap.value,
...
const allRecipeCategories = computed(() =>
  [...new Set(allRecipesSummary.value.map(r => r.Category).filter(Boolean))].sort()
)
...
const recipeStockPreview = computed(() => {
  ...
  const invUnitMap = Object.fromEntries(rawProducts.value.map(r => [r.rawproductid, r.unit]))
  const fakeRecipes = recipeRows.value
    ...
    .map(r => ({
      ...
      unit: r.unit,
      rawproduct: rawProducts.value.find(rp => rp.rawproductid === r.rawproductid),
    }))
  return checkStockForProduct(activeItem.value.ProductId, 1, fakeRecipes, stockMap.value, invUnitMap)
})
...
const fetchAllRecipes = async () => {
  ...
  allRecipes.value = (data ?? []).map(r => ({ ...r, rawproduct: r.rawproduct }))
}

const fetchMenuItems = async (force = false) => {
  if (!force) {
    const cached = loadCache(CACHE_KEY_MENU)
    if (cached) { menuItems.value = cached; return }
  }
  loading.value = true
  const { data: products, error } = await supabase
    .from('product')
    .select('ProductId, ProductName, ProductType, Category, Price, size_prices, image_url, Status')
    .neq('Status', 'Archived')
  ...
  const { data: recipes } = await supabase.from('recipe').select('finishedproductid')
  const recipeCounts = {}
  ;(recipes ?? []).forEach(r => {
    recipeCounts[r.finishedproductid] = (recipeCounts[r.finishedproductid] || 0) + 1
  })

  // Enrich with stock detail
  const invUnitMap = Object.fromEntries(rawProducts.value.map(r => [r.rawproductid, r.unit]))
  menuItems.value = (products ?? []).map(p => {
    const count = recipeCounts[p.ProductId] ?? 0
    let stockDetail = null
    if (count > 0) {
      stockDetail = checkStockForProduct(p.ProductId, 1, allRecipes.value, stockMap.value, invUnitMap)
    }
    return { ...p, _recipeCount: count, _stockDetail: stockDetail }
  })
  saveCache(CACHE_KEY_MENU, menuItems.value)
  loading.value = false
}
...
const onCategoryChange = () => {
  if (getSizeType(form.value.Category) !== 'none') {
    const newSizePrices = {}
    getSizeLabels(form.value.Category).forEach(s => { newSizePrices[s] = null })
...
const openAddModal = () => {
  ...
  form.value = { ProductName: '', Category: '', Price: null, sizePrices: {}, image_url: null }
  showModal.value = true
}

const openEditModal = async (item) => {
  isEditing.value = true; editingId.value = item.ProductId
  form.value = { ProductName: item.ProductName, Category: item.Category ?? '', Price: item.Price, sizePrices: item.size_prices || {} }
  showModal.value = true
}
...
const handleImageUpload = async (e) => {
  ...
  if (!file) return
  try { form.value.image_url = await uploadImage(file) }
...
const saveItem = async () => {
  if (!form.value.ProductName) { alert('Product name is required.'); return }
  if (!form.value.Category)    { alert('Please select a category.'); return }
  const sizeLabels = getSizeLabels(form.value.Category)
  if (sizeLabels.length > 0) {
    for (const size of sizeLabels) {
      if (!form.value.sizePrices[size] || form.value.sizePrices[size] <= 0) { alert(`Please enter a price for ${size}`); return }
    }
  }
  saving.value = true
  const payload = {
    ProductName: form.value.ProductName, ProductType: 'finished',
  ...
  let error
  if (isEditing.value) {
    const { error: e } = await supabase.from('product').update(payload).eq('ProductId', editingId.value)
    error = e
  } else {
  ...
  if (error) { alert('Failed to save: ' + error.message) }
    ...
    await Promise.all([fetchAllRecipes(), fetchMenuItems(true)])
    closeModal()
  }
  saving.value = false
}
...
const deleteItem = async (id) => {
  ...
  const { error } = await supabase.from('product').update({ Status: 'Archived', ArchivedAt: new Date().toISOString(), ArchivedBy: currentUser }).eq('ProductId', id)
  if (error) { alert('Failed to archive: ' + error.message) }
  else { sessionStorage.removeItem(CACHE_KEY_MENU); await fetchMenuItems(true) }
}

// ─── Recipe CRUD ──────────────────────────────────────────────────────────────
const openRecipeModal = async (item) => {
  activeItem.value = item; recipeRows.value = []; loadingRecipe.value = true; showRecipeModal.value = true
  const { data, error } = await supabase.from('recipe').select('recipeid, rawproductid, quantityneeded, unit').eq('finishedproductid', item.ProductId).order('rawproductid')
  loadingRecipe.value = false
  if (!error && data && data.length > 0) {
    recipeRows.value = data.map(r => ({ rawproductid: r.rawproductid, quantityneeded: r.quantityneeded, unit: r.unit ?? suggestRecipeUnit(r.rawproductid, rawProducts.value) }))
  } else {
...
const saveRecipe = async () => {
  ...
  if (validRows.length === 0) { alert('Please add at least one ingredient with a valid quantity.'); return }
  savingRecipe.value = true
  const { error: delErr } = await supabase.from('recipe').delete().eq('finishedproductid', activeItem.value.ProductId)
  if (delErr) { alert('Failed to update recipe: ' + delErr.message); savingRecipe.value = false; return }
  ...
  if (insErr) { alert('Failed to save recipe: ' + insErr.message) }
  else {
    sessionStorage.removeItem(CACHE_KEY_MENU)
    await Promise.all([fetchAllRecipes(), fetchMenuItems(true)])
    closeRecipeModal()
...
const openAllRecipes = async () => {
  showAllRecipesModal.value = true; loadingAllRecipes.value = true; recipeSearch.value = ''; expandedRecipe.value = null
  const { data: recipes } = await supabase.from('recipe').select('recipeid, finishedproductid, rawproductid, quantityneeded, unit')
  const { data: raws } = await supabase.from('rawproduct').select('rawproductid, name, unit')
  const rawMap = {}; (raws ?? []).forEach(r => { rawMap[r.rawproductid] = r })
  const productIds = [...new Set((recipes ?? []).map(r => r.finishedproductid))]
  if (productIds.length === 0) { allRecipesSummary.value = []; loadingAllRecipes.value = false; return }
  const { data: products } = await supabase.from('product').select('ProductId, ProductName, Category, Price').in('ProductId', productIds).order('Category').order('ProductName')
  allRecipesSummary.value = (products ?? []).map(p => ({
    ...p,
    recipes: (recipes ?? []).filter(r => r.finishedproductid === p.ProductId).map(r => ({
      ...r, rawName: rawMap[r.rawproductid]?.name ?? '—', rawUnit: rawMap[r.rawproductid]?.unit ?? '',
    })),
  }))
  if (allRecipeCategories.value.length > 0) activeRecipeCategory.value = allRecipeCategories.value[0]
...
/* LEGEND BAR */
.stock-legend-bar { display: flex; align-items: center; gap: 14px; background: white; border: 1px solid #eee; border-radius: 9px; padding: 9px 14px; margin-bottom: 16px; font-size: 12px; flex-wrap: wrap; }
.leg-item { display: flex; align-items: center; gap: 5px; color: #555; font-weight: 600; }
.leg-dot { width: 10px; height: 10px; border-radius: 50%; }
.leg-dot.oos { background: #dc2626; }
.leg-dot.low { background: #f59e0b; }
.leg-dot.recipe-ok { background: #16a34a; }
.leg-dot.no-recipe { background: #d1d5db; border: 1px dashed #9ca3af; }
.leg-sep { color: #ddd; }
.leg-note { color: #aaa; font-weight: 400; }
...
.filter-sel { appearance: none; padding: 9px 32px 9px 12px; border: 1px solid #eee; border-radius: 8px; font-size: 14px; background: white; outline: none; cursor: pointer; min-width: 150px; }
...
.section-title { font-size: 16px; font-weight: 700; color: #888; margin-bottom: 14px; text-transform: uppercase; letter-spacing: 0.05em; display: flex; align-items: center; gap: 8px; }
...
.section-oos-warn { display: flex; align-items: center; gap: 4px; background: #fee2e2; color: #dc2626; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 20px; text-transform: none; letter-spacing: 0; }

/* GRID */
.menu-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; }

/* CARD */
.menu-card { background: white; border: 1.5px solid #eee; border-radius: 14px; padding: 0; transition: box-shadow 0.2s; display: flex; flex-direction: column; gap: 10px; position: relative; overflow: hidden; }
.menu-card:hover { box-shadow: 0 4px 16px rgba(0,0,0,0.06); }
.menu-card.stock-oos { border-color: #fca5a5; background: #fff5f5; }
.menu-card.stock-low { border-color: #fcd34d; }
.menu-card.no-recipe-card { border-style: dashed; border-color: #d1d5db; }

/* STOCK RIBBON */
.stock-ribbon { display: flex; align-items: center; gap: 5px; font-size: 11px; font-weight: 700; padding: 4px 10px; }
.stock-ribbon span { display: flex; align-items: center; gap: 4px; }
.ribbon-available { background: #d1fae5; color: #065f46; }
.ribbon-low { background: #fef3c7; color: #92400e; }
.ribbon-oos { background: #fee2e2; color: #991b1b; }
.ribbon-no_recipe { background: #f3f4f6; color: #6b7280; }

/* CARD BODY */
.card-image { width: 100%; height: 120px; background: #f5f0eb; display: flex; align-items: center; justify-content: center; overflow: hidden; }
...
.name-row h4 { margin: 0; font-size: 16px; font-weight: 700; color: #31201d; }
.size-prices { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 4px; }
.size-price-tag { font-size: 13px; font-weight: 600; color: #6a9c7a; background: #f0faf3; padding: 2px 8px; border-radius: 4px; border: 1px solid #c8e6d0; }

/* STOCK DETAIL */
.stock-detail-row { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 6px; }
.shortage-chip { display: flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 700; background: #fee2e2; color: #dc2626; padding: 2px 7px; border-radius: 4px; }

/* RECIPE BUTTON */
.recipe-status-area { padding: 0 14px; }
...
.recipe-btn.add { border: 1.5px dashed #e5e7eb; background: #f9fafb; color: #9ca3af; }
.recipe-btn.add:hover { border-color: #c49a6c; color: #c49a6c; background: #fff9f0; }

/* PRICE ROW */
.price-action-row { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #f5f5f5; padding: 10px 14px; }
...
.modal-content.recipe-modal { width: 640px; }
...
/* SIZE PRICES */
.sizes-container { display: flex; flex-direction: column; gap: 10px; margin-top: 8px; }
.size-price-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; background: #f9f4ef; border-radius: 8px; }
.size-name { font-weight: 700; color: #31201d; font-size: 14px; }
.size-price-input { display: flex; align-items: center; gap: 5px; }
.size-price-input .currency { font-weight: 600; color: #31201d; }
.size-price-input input { width: 120px; padding: 8px 10px; border: 1px solid #e8e0d5; border-radius: 6px; font-size: 14px; text-align: right; }
.preview-img { width: 100%; max-height: 160px; object-fit: cover; border-radius: 8px; margin-bottom: 8px; }
.upload-label { display: inline-block; padding: 8px 16px; background: #f5f0eb; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 600; color: #31201d; }
.upload-label:hover { background: #ede5d8; }

/* RECIPE STOCK PREVIEW */
.recipe-stock-preview { background: #fff5f5; border: 1px solid #fecaca; border-radius: 10px; padding: 12px 14px; margin-bottom: 14px; }
.rsp-header { display: flex; align-items: center; gap: 7px; font-size: 13px; font-weight: 700; color: #dc2626; margin-bottom: 8px; }
.rsp-row { display: flex; justify-content: space-between; font-size: 12px; padding: 4px 0; border-bottom: 1px solid #fee2e2; }
.rsp-row:last-child { border-bottom: none; }
.rsp-name { font-weight: 600; color: #31201d; }
.rsp-detail { color: #dc2626; font-weight: 600; }
.recipe-stock-ok { display: flex; align-items: center; gap: 7px; background: #d1fae5; color: #065f46; font-size: 13px; font-weight: 700; padding: 9px 13px; border-radius: 8px; margin-bottom: 14px; }

/* INGREDIENT TABLE */
.ingredient-header { display: grid; grid-template-columns: 2fr 1fr 1.2fr 32px; gap: 8px; margin-bottom: 8px; padding: 0 2px; }
.ingredient-header label { font-size: 11px; font-weight: 700; color: #aaa; text-transform: uppercase; letter-spacing: 0.04em; display: flex; align-items: center; gap: 4px; }
.unit-auto-tag { background: #0369a1; color: white; font-size: 9px; font-weight: 800; padding: 1px 5px; border-radius: 3px; text-transform: none; letter-spacing: 0; }
.ingredient-list { max-height: 320px; overflow-y: auto; padding: 2px; }
.ingredient-row { display: grid; grid-template-columns:2fr 1fr 1.2fr 32px; gap: 8px; margin-bottom: 10px; align-items: start; }
.ingredient-row input { width: 100%; height: 40px; padding: 0 10px; border: 1px solid #eee; border-radius: 8px; background: #fafafa; font-size: 14px; outline: none; transition: 0.2s; box-sizing: border-box; }
.ingredient-row input:focus { border-color: #c49a6c; background: white; }
.ingredient-row .select-wrap.full select { height: 40px; padding: 0 24px 0 8px; font-size: 13px; }
.unit-field { display: flex; flex-direction: column; gap: 3px; }
.convert-hint { font-size: 10px; color: #0369a1; font-weight: 600; padding: 0 2px; }
.stock-col { display: flex; align-items: flex-start; padding-top: 10px; }
.ing-stock { font-size: 11px; font-weight: 700; }
.ing-stock.ok { color: #16a34a; }
.ing-stock.warn { color: #d97706; }
.ing-stock.danger { color: #dc2626; }
.ing-stock.neutral { color: #888; }
.ing-stock-na { font-size: 11px; color: #ccc; padding-top: 10px; }
.remove-ing { background: none; border: none; color: #dc2626; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 8px 4px; border-radius: 6px; transition: 0.2s; }
...
/* ALL RECIPES MODAL */
.modal-content.all-recipes-modal { width: 780px; max-width: 95vw; padding: 28px 0 0; overflow: hidden; display: flex; flex-direction: column; max-height: 88vh; }
.modal-content.all-recipes-modal .modal-hdr { padding: 0 28px 20px; border-bottom: 1px solid #f0f0f0; margin-bottom: 0; }
.all-recipes-layout { display: flex; flex: 1; overflow: hidden; }
.recipe-sidebar { width: 200px; flex-shrink: 0; border-right: 1px solid #f0f0f0; overflow-y: auto; padding: 12px 8px; display: flex; flex-direction: column; gap: 3px; }
.cat-tab { display: flex; align-items: center; justify-content: space-between; width: 100%; padding: 9px 12px; border: none; border-radius: 8px; background: none; cursor: pointer; font-size: 13px; font-weight: 500; color: #666; text-align: left; transition: 0.15s; gap: 8px; }
.cat-tab:hover { background: #f5f5f5; color: #31201d; }
.cat-tab.active { background: #fff9f0; color: #31201d; font-weight: 700; }
.cat-tab-name { flex: 1; }
.cat-tab-count { font-size: 11px; font-weight: 700; background: #f0f0f0; color: #888; padding: 1px 7px; border-radius: 10px; min-width: 20px; text-align: center; }
...
.recipe-search { padding: 14px 16px; border-bottom: 1px solid #f5f5f5; display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.recipe-search svg { color: #bbb; flex-shrink: 0; }
.recipe-search input { flex: 1; border: none; outline: none; font-size: 13px; background: none; color: #31201d; }
.recipe-panel-scroll { flex: 1; overflow-y: auto; padding: 10px 12px 16px; display: flex; flex-direction: column; gap: 6px; }
.recipe-card { border: 1px solid #eee; border-radius: 10px; overflow: hidden; cursor: pointer; transition: border-color 0.15s, box-shadow 0.15s; }
.recipe-card:hover { border-color: #c49a6c44; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
.recipe-card.expanded { border-color: #c49a6c; }
.recipe-card-header { display: flex; align-items: center; justify-content: space-between; padding: 12px 14px; background: white; gap: 10px; }
...
.recipe-ing-pill { font-size: 11px; font-weight: 600; background: #f0f0f0; color: #888; padding: 2px 7px; border-radius: 10px; flex-shrink: 0; }
...
.recipe-card-body { padding: 0 14px 12px; background: #fdfaf7; border-top: 1px solid #f5ede0; display: flex; flex-direction: column; gap: 5px; }
.ing-row { display: flex; align-items: center; gap: 8px; padding: 5px 0; border-bottom: 1px solid #f0ebe0; font-size: 12px; }
...
.ing-inv-unit { font-size: 10px; color: #0369a1; font-weight: 600; background: #dbeafe; padding: 1px 5px; border-radius: 3px; }

@media (max-width: 1024px) { .menu-grid { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 640px) { .menu-grid { grid-template-columns: 1fr; } .all-recipes-layout { flex-direction: column; } .recipe-sidebar { width: 100%; flex-direction: row; overflow-x: auto; border-right: none; border-bottom: 1px solid #f0f0f0; } }
</style>