  ...
  <div class="pos-root">
    ...
    <div class="pos-layout">
      ...
      <aside class="cart-panel">
        ...
        <div class="order-type-row">
          <button
            :class="['order-type-btn', orderType === 'dine_in' ? 'active-dine' : '']"
            @click="orderType = 'dine_in'"
          >
            <UtensilsCrossed :size="13" /> Dine In
          </button>
          <button
            :class="['order-type-btn', orderType === 'take_out' ? 'active-take' : '']"
            @click="orderType = 'take_out'"
          >
            <ShoppingBag :size="13" /> Take Out
          </button>
        </div>
        ...
        <div v-if="cart.length > 0" class="cart-discount">
          ...
          <div v-if="selectedDiscount" class="discount-id-field">
            <label class="discount-id-label">
              <CreditCard :size="12" />
              {{ getDiscountIdLabel(selectedDiscount) }} *
            </label>
            <input
    ...
    <div v-if="showOosAlert" class="overlay" @click.self="showOosAlert = false">
      <div class="oos-modal">
        ...
        <p><strong>{{ oosProductName }}</strong> cannot be prepared due to insufficient raw materials:</p>
        <div class="oos-list">
          <div v-for="item in oosIngredients" :key="item.name" class="oos-item">
        ...
        <div v-if="showReceipt" class="receipt-view">
          ...
          <div id="print-receipt" class="receipt-paper">
            ...
            <div class="r-meta">
              ...
              <div v-if="selectedDiscount" class="r-row">
                <span>Discount ({{ selectedDiscount.discountname }})</span>
                <span>{{ discountIdNumber || 'N/A' }}</span>
              </div>
            </div>
        ...
        <div v-else class="payment-view">
          ...
          <div v-if="paymentMethod === 'cash'" class="cash-fields">
            ...
            <button class="qa-btn exact-btn" @click="cashReceived = finalTotal">
              ₱{{ finalTotal.toFixed(2) }} <span class="exact-label">Exact Amount</span>
            </button>
      ...
      <div class="history-modal">
        ...
        <div v-else class="hm-table-wrap">
          <table class="hm-table">
            <thead>
              <tr>
                <th>Invoice #</th><th>Time</th><th>Items</th>
                <th>Type</th><th>Payment</th><th>Discount</th><th>Total</th><th>Status</th>
                <th>Action</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="tr in transactions" :key="tr.OrderId" :class="{ 'cancelled-row': tr.Status === 'cancelled' }">
                ...
                <td>
                  <span :class="['order-type-badge', tr.order_type]">
                    {{ tr.order_type === 'take_out' ? 'Take Out' : 'Dine In' }}
                  </span>
                </td>
                <td>
                ...
                <td>
                  <div v-if="tr.discount">
                    <span class="disc-badge">{{ tr.discount.discountname }}</span>
                    <div v-if="tr.discount_id_number" class="disc-id-display">ID: {{ tr.discount_id_number }}</div>
                  </div>
                ...
                <td>
                  ...
                  <div v-if="tr.Status === 'cancelled' && tr.cancel_reason" class="cancel-reason-display">
                    "{{ tr.cancel_reason }}"
                  </div>
                </td>
                <td>
        ...
        <div class="cancel-reason-field">
          ...
          <div class="cancel-reason-presets">
            <button
              ...
              :key="preset"
              :class="['preset-btn', cancelReason === preset ? 'selected' : '']"
...
// Raw material stock cache (fetched alongside menu)
const rawMaterialStock = ref({}) // rawproductid → available qty
const productRecipes = ref([])   // all recipes: { finishedproductid, rawproductid, quantityneeded, unit }
...
const orderType = ref('dine_in') // 'dine_in' | 'take_out'
...
// Subtotal before VAT adjustments (prices already VAT-inclusive per PH standard)
...
// VAT computed on amount after discount (VAT-inclusive model: VAT = total × 12/112)
...
const getDiscountIdLabel = (discount) => {
  if (!discount) return 'ID Number'
  const name = discount.discountname?.toLowerCase() ?? ''
  if (name.includes('pwd') || name.includes('person with disability')) return 'PWD ID Number'
  if (name.includes('senior')) return 'Senior Citizen ID Number'
  return 'Discount ID Number'
}

const getDiscountIdPlaceholder = (discount) => {
  if (!discount) return 'Enter ID number'
  const name = discount.discountname?.toLowerCase() ?? ''
  if (name.includes('pwd')) return 'e.g. PWD-2024-000001'
  if (name.includes('senior')) return 'e.g. SC-2024-000001'
  return 'Enter ID / card number'
}

// ── Stock check ───────────────────────────────────────────────────────────────
/**
 * Returns an array of { name, needed, available, unit } for ingredients that
 * are insufficient. Empty array = product can be made.
 */
const checkStockForProduct = (productId, qty = 1) => {
  const recipes = productRecipes.value.filter(r => r.finishedproductid === productId)
  if (recipes.length === 0) return [] // no recipe = no restriction

  const insufficient = []
  for (const recipe of recipes) {
    const needed = recipe.quantityneeded * qty
    const available = rawMaterialStock.value[recipe.rawproductid] ?? 0
    if (available < needed) {
      insufficient.push({
        name: recipe.rawName ?? `Raw #${recipe.rawproductid}`,
        needed,
        available,
        unit: recipe.unit ?? '',
      })
...
const isProductUnavailable = (item) => {
  return checkStockForProduct(item.ProductId, 1).length > 0
}
...
const handleMenuCardClick = (item) => {
  ...
  const insufficient = checkStockForProduct(item.ProductId, 1)
  if (insufficient.length > 0) {
    oosProductName.value = item.ProductName
    oosIngredients.value = insufficient
...
const increaseQty = (i) => {
  ...
  // Check if adding one more is still possible
  const newQty = item.qty + 1
  const insufficient = checkStockForProduct(item.ProductId, newQty)
  if (insufficient.length > 0) {
    oosProductName.value = item.ProductName
    oosIngredients.value = insufficient
    showOosAlert.value = true
...
/**
 * Fetch menu + recipes + raw material stock together so all three
 * views (Menu, POS, Inventory) read from the same source of truth.
 */
const fetchMenu = async () => {
  ...
  if (error) { console.error('fetchMenu error:', error); loadingMenu.value = false; return }
  menu.value = products ?? []

  // 2. Fetch all active recipes (only for non-archived raw products)
  const { data: recipes } = await supabase
    ...
    `)
    // Exclude archived raw products from recipes
    .neq('rawproduct.status', 'Archived')

  // Also fetch raw product names for OOS display
  productRecipes.value = (recipes ?? []).map(r => ({
    ...r,
    rawName: r.rawproduct?.name ?? `Raw #${r.rawproductid}`,
  }))

  // 3. Fetch raw material stock for this branch
  await fetchRawStock()
...
const fetchRawStock = async () => {
  ...
  // Compute net stock per raw product
  const stock = {}
  for (const t of txns) {
...
// ── Payment ───────────────────────────────────────────────────────────────────
const openPayment = () => {
  // Validate discount ID if discount selected
...
const goToInvoicePreview = () => {
  // Re-check stock before proceeding to invoice
  for (const item of cart.value) {
    const insufficient = checkStockForProduct(item.ProductId, item.qty)
    if (insufficient.length > 0) {
...
const printReceipt = () => {
  ...
  printWin.document.write(`
    <html>
      <head>
        <title>Sales Invoice #${transactionNumber.value} - Silingan Coffee</title>
        <style>
          * { margin:0; padding:0; box-sizing:border-box; }
          ...
          .r-invoice-title { font-size:13px; font-weight:bold; text-transform:uppercase; letter-spacing:2px; margin:8px 0 4px; border:1px solid #000; padding:3px 6px; display:inline-block; }
          .r-txn-no { font-weight:bold; }
          ...
          .r-items-header { display:grid; grid-template-columns:3fr 1fr 2fr 2fr; font-size:10px; font-weight:bold; margin-bottom:4px; padding-bottom:3px; border-bottom:1px solid #000; }
          .r-item { display:grid; grid-template-columns:3fr 1fr 2fr 2fr; margin-bottom:3px; font-size:11px; }
          .r-item-name { font-weight:500; }
          .r-total { font-weight:bold; font-size:13px; color:#000; padding-top:4px; border-top:1px solid #000; margin-top:3px; }
          .r-change { font-weight:bold; }
          .r-vat { color:#555; }
          .r-footer { text-align:center; margin-top:10px; font-size:11px; color:#777; }
          .r-vat-note { font-size:9px; color:#777; margin-top:8px; text-align:center; border-top:1px dashed #ccc; padding-top:5px; }
...
// ── Deduct inventory ──────────────────────────────────────────────────────────
const deductInventoryForOrder = async (cartItems, bid) => {
  const productIds = cartItems.map(i => i.ProductId)
  const recipes = productRecipes.value.filter(r => productIds.includes(r.finishedproductid))
  ...
  const txns = Object.entries(deductions).map(([rawproductid, qty]) => ({
  ...
  const { error } = await supabase.from('rawproducttransaction').insert(txns)
  if (error) console.error('Inventory deduction failed (order saved):', error.message)
}
...
const doCancelOrder = async () => {
  ...
  try {
    const { error } = await supabase
      .from('orders')
      .update({ Status: 'cancelled', cancel_reason: cancelReason.value.trim() })
...
.oos-overlay { position:absolute; inset:0; background:rgba(220,38,38,0.82); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:5px; color:white; font-size:12px; font-weight:700; }
...
/* OOS MODAL */
.oos-modal { background:white; border-radius:16px; width:400px; max-width:95vw; padding:28px; text-align:center; box-shadow:0 20px 60px rgba(0,0,0,0.2); }
...
.oos-list { background:#fff5f5; border:1px solid #fecaca; border-radius:10px; padding:12px 14px; text-align:left; margin-bottom:18px; display:flex; flex-direction:column; gap:7px; }
.oos-item { display:flex; align-items:center; gap:8px; font-size:12px; }
.oos-dot { width:6px; height:6px; border-radius:50%; background:#dc2626; flex-shrink:0; }
...
.r-vat-note { font-size:9px; color:#999; margin-top:10px; text-align:center; border-top:1px dashed #ddd; padding-top:8px; line-height:1.4; }
