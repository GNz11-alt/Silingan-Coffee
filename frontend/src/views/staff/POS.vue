  ...
  <div class="pos-root">
        ...
        <div v-else class="menu-grid">
          <button
            ...
            :class="{
              ...
              'low-stock-card':
                !isProductUnavailable(item) && isProductLow(item),
            }"
          ...
          >
            <div class="menu-card-img">
              ...
              <div v-if="isProductUnavailable(item)" class="oos-overlay">
                ...
                <span class="oos-overlay-detail">{{
                  getOOSShortLabel(item)
                }}</span>
              </div>
      ...
      <aside class="cart-panel">
        ...
        <div class="order-type-row">
          ...
          <button
            :class="[
              'order-type-btn',
              orderType === 'take_out' ? 'active-take' : '',
            ]"
        ...
        <div class="cart-items" v-if="cart.length > 0">
          <div v-for="(item, i) in cart" :key="i" class="cart-item">
            ...
            <div class="ci-price-row">
              <span class="ci-price"
                >₱{{ item.effectivePrice?.toFixed(2) }} each</span
              >
            </div>
            ...
            <div class="ci-subtotal">
              ₱{{ (item.qty * item.effectivePrice).toFixed(2) }}
            </div>
          </div>
        </div>
        ...
        <div v-if="cart.length > 0" class="cart-discount">
          ...
          <div v-if="selectedDiscount" class="discount-id-field">
            ...
            <input
              ...
              maxlength="30"
            />
            <span v-if="discountIdError" class="discount-id-error">{{
              discountIdError
            }}</span>
          </div>
        </div>
    ...
    <div v-if="showOosAlert" class="overlay" @click.self="showOosAlert = false">
      <div class="oos-modal">
        ...
        <button class="oos-close-btn" @click="showOosAlert = false">
          OK, Got It
        </button>
      </div>
    </div>
    ...
    >
      <div class="size-modal">
        <div class="size-modal-header">
          ...
          <button class="pv-close" @click="showSizePicker = false">
            <X :size="19" />
          </button>
        </div>
      ...
      <div class="payment-modal">
          ...
          <div id="print-receipt" class="receipt-paper">
            ...
            <div class="r-meta">
              <div class="r-row">
                ...
                ><span class="r-txn-no">#{{ transactionNumber }}</span>
              </div>
              <div class="r-row">
        ...
        <div v-else class="payment-view">
          <div class="pv-header">
            ...
            <button class="pv-close" @click="showPayment = false">
              <X :size="19" />
            </button>
          </div>
          <div class="pm-tabs">
    ...
    <div v-if="showHistory" class="overlay" @click.self="showHistory = false">
      <div class="history-modal">
        <div class="hm-header">
          ...
          <button class="pv-close" @click="showHistory = false">
            <X :size="19" />
          </button>
        </div>
        <div class="hm-stats">
          ...
          <table class="hm-table">
            ...
            <tbody>
              ...
              >
                <td>
                  <span class="id-tag"
                    >#{{ tr.transaction_number || tr.OrderId }}</span
                  >
                </td>
                <td class="muted">{{ formatTime(tr.CreatedAt) }}</td>
                <td>
                  <div
                    v-for="it in tr.orderitem"
                    :key="it.OrderItemId"
                    class="h-item-line"
                  >
    ...
    >
      <div class="confirm-modal">
        ...
        <h3>
          Cancel Order #{{
            cancelTarget?.transaction_number || cancelTarget?.OrderId
          }}?
        </h3>
        <p>This will mark the order as cancelled. Please provide a reason.</p>
        <div class="confirm-items">
          <div
            v-for="it in cancelTarget?.orderitem"
            :key="it.OrderItemId"
            class="confirm-item-line"
          >
            {{ it.Quantity }}x {{ it.product?.ProductName ?? "—" }}
          </div>
        </div>
        <div class="confirm-total">
          Total: ₱{{ (cancelTarget?.FinalAmount ?? 0).toFixed(2) }}
        </div>
        <div class="cancel-reason-field">
          ...
          <textarea
            ...
            rows="2"
          ></textarea>
          <span v-if="cancelReasonError" class="cancel-reason-error">{{
            cancelReasonError
          }}</span>
        </div>
...
const CACHE_KEYS = {
  menu: "pos_cache_menu",
  recipes: "pos_cache_recipes",
  stock: "pos_cache_stock",
  unitMap: "pos_cache_unitMap",
  discounts: "pos_cache_discounts",
  transactions: "pos_cache_transactions",
};
const CACHE_TTL = {
  menu: 30 * 60 * 1000, // 30 min
  recipes: 5 * 60 * 1000, // 5 min
  stock: 1 * 60 * 1000, // 1 min  (stock changes often)
  discounts: 10 * 60 * 1000, // 10 min
  transactions: 30 * 1000, // 30 sec (changes every sale)
};
...
const filteredMenu = computed(() => {
  ...
  if (menuSearch.value)
    list = list.filter((i) =>
      i.ProductName?.toLowerCase().includes(menuSearch.value.toLowerCase()),
    );
...
const afterDiscountAmount = computed(() =>
  Math.max(0, cartSubtotal.value - discountAmount.value),
);
const vatAmount = computed(() =>
  parseFloat(
    ((afterDiscountAmount.value * VAT_RATE) / (1 + VAT_RATE)).toFixed(2),
  ),
);
...
const fetchCurrentUser = async () => {
  ...
  if (u?.employee_id) {
    const { data: emp } = await supabase
      .from("employee")
      .select("EmployeeId")
      .eq("EmployeeId", u.employee_id)
      .maybeSingle();
    if (emp) {
      employeeRecord.value = emp;
      return;
    }
  }
  if (username) {
...
const fetchMenu = async (force = false) => {
  if (!force) {
    ...
    if (cachedMenu && cachedRecipes && cachedStock && cachedUnitMap) {
      ...
      return; // no loadingMenu = true, no flash
    }
  }
...
const fetchDiscounts = async (force = false) => {
  if (!force) {
    const cached = readCache(CACHE_KEYS.discounts);
    if (cached) {
  ...
  if (data) {
    ...
    writeCache(CACHE_KEYS.discounts, discounts.value, CACHE_TTL.discounts);
  }
};
...
const finishTransaction = async () => {
  ...
  try {
    const payload = {
      ...
      cashpaid:
        paymentMethod.value === "cash" ? cashReceived.value : finalTotal.value,
      changegiven:
        paymentMethod.value === "cash" ? Math.max(0, changeAmount.value) : 0,
      transaction_number: transactionNumber.value,
      discount_id_number: selectedDiscount.value
        ? discountIdNumber.value.trim() || null
        : null,
    ...
    // ── FEFO deduction then bust + refresh ────────────────────────────────
    await deductInventoryFEFO(cart.value, branchId.value); // ← was missing!
    bustCache(CACHE_KEYS.stock, CACHE_KEYS.unitMap, CACHE_KEYS.transactions);
    await fetchRawStock(true); // force-refresh stock
    await fetchTransactions(true); // force-refresh today's sales
    // ← remove the extra fetchTransactions() that was here
...
const deductInventoryFEFO = async (cartItems, bid) => {
  try {
    ...
    if (error)
      console.error("FEFO deduction error (order saved):", error.message);
  } catch (err) {
...
const doCancelOrder = async () => {
  ...
  try {
    ...
    const tx = transactions.value.find(
      (t) => t.OrderId === cancelTarget.value.OrderId,
    );
    if (tx) {
      tx.Status = "cancelled";
    ...
    // The local optimistic update you already have is fine — no re-fetch needed.
    closeCancelModal();
  } catch (err) {
...
const getCatIcon = (cat) => {
  ...
  if (c.includes("pastry") || c.includes("rice") || c.includes("pika"))
    return Cookie;
...
.pos-layout {
  display: grid;
  grid-template-columns: 152px 1fr 320px;
  flex: 1;
  overflow: hidden;
}
.category-sidebar {
...
.oos-filter-btn:hover {
  border-color: #31201d;
  color: #31201d;
}
.oos-filter-btn.active {
  background: #31201d;
  color: white;
  border-color: #31201d;
}
...
.menu-card:hover:not(.out-of-stock-card) {
...
.menu-card.out-of-stock-card {
  opacity: 0.65;
  cursor: not-allowed;
}
.menu-card.out-of-stock-card:hover {
  transform: none;
  box-shadow: none;
  border-color: transparent;
}
.menu-card.low-stock-card {
  border-color: #fbbf24;
}
.menu-card-img {
  background: #f9f4ef;
...
.order-type-btn.active-dine {
  background: #31201d;
  color: white;
  border-color: #31201d;
}
.order-type-btn.active-take {
  background: #c49a6c;
  color: white;
  border-color: #c49a6c;
}
...
.gcash-badge {
  background: linear-gradient(135deg, #0064e0 0%, #00a3ff 100%);
  border-radius: 14px;
  padding: 30px;
  text-align: center;
  color: white;
...
.rbtn-confirm:hover:not(:disabled) {
  background: #4a3330;
}
.rbtn-confirm:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
.history-modal {
