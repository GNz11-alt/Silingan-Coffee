  ...
  <div class="pos-root">
    ...
    <header class="pos-topbar">
      <div class="topbar-left">
        <div class="brand-mark">
          <Coffee :size="20" /><span>Silingan POS</span>
        </div>
        <div class="topbar-meta">
          <span class="meta-branch">{{
            branchAddress || branchRecord?.BranchName || "Main Branch"
          }}</span>
      ...
      <div class="topbar-right">
        <button class="history-btn" @click="openHistory">
          ...
          <span v-if="transactions.length > 0" class="history-count">{{
            transactions.length
          }}</span>
        </button>
      </div>
    ...
    <div class="pos-layout">
      ...
      <aside class="category-sidebar">
        ...
        <button
          v-for="cat in ['All', ...menuCategories]"
          :key="cat"
      ...
      <main class="menu-area">
        ...
        <div v-else-if="filteredMenu.length === 0" class="menu-empty">
          No items found.
        </div>
        <div v-else class="menu-grid">
          ...
          >
            <div class="menu-card-img">
              ...
              <span
                v-if="getSizeType(item.Category) !== 'none'"
                class="size-chip"
                >{{ getSizeLabels(item.Category).join(" / ") }}</span
              >
            ...
            <div class="menu-card-info">
              ...
              <span class="menu-card-price">
                <template v-if="getSizeType(item.Category) !== 'none'"
                  >from ₱{{ getBasePrice(item) }}</template
                >
    ...
    <div v-if="showOosAlert" class="overlay" @click.self="showOosAlert = false">
      <div class="oos-modal">
        ...
        <p>
          <strong>{{ oosProductName }}</strong> cannot be prepared —
        ...
        <div class="oos-list">
          <div v-for="item in oosIngredients" :key="item.rawproductid" class="oos-item">
            ...
            <span class="oos-qty">
              Need {{ formatQtyDisplay(item.needed, item.unit) }}, have
              {{ formatQtyDisplay(item.available, item.unit) }}
            </span>
          </div>
        </div>
      ...
      <div class="payment-modal">
        ...
        <div v-if="showReceipt" class="receipt-view">
          <div id="print-receipt" class="receipt-paper">
            ...
            <div class="r-meta">
              <div class="r-row">
                <span>Invoice No.</span><span class="r-txn-no">#{{ transactionNumber }}</span>
              </div>
              <div class="r-row"><span>Date</span><span>{{ currentDate }}</span></div>
              <div class="r-row"><span>Cashier</span><span>{{ cashierName }}</span></div>
              <div class="r-row">
                <span>Order Type</span>
                <span class="r-order-type">{{ orderType === "dine_in" ? "Dine In" : "Take Out" }}</span>
              </div>
              <div class="r-row">
                <span>Payment</span>
                <span>{{ paymentMethod === "gcash" ? "GCash" : "Cash" }}</span>
              </div>
            ...
            <div class="r-items">
              <div v-for="item in cart" :key="item.cartKey" class="r-item">
                <span class="r-item-name">
                  {{ item.ProductName }}<span v-if="item.size"> ({{ item.size }})</span>
                </span>
                <span>{{ item.qty }}</span>
            ...
            <div class="r-totals">
              <div class="r-row">
                <span>Subtotal (VAT-incl.)</span><span>₱{{ cartSubtotal.toFixed(2) }}</span>
              </div>
              <div class="r-row r-disc" v-if="selectedDiscount">
                <span>Discount ({{ selectedDiscount.discountname }})</span>
                <span>-₱{{ discountAmount.toFixed(2) }}</span>
              </div>
              <div class="r-row r-vat">
                <span>VAT (12%)</span><span>₱{{ vatAmount.toFixed(2) }}</span>
              </div>
              ...
              <template v-if="paymentMethod === 'cash'">
                ...
                <div class="r-row r-change">
                  <span>Change</span><span>₱{{ Math.max(0, changeAmount).toFixed(2) }}</span>
                </div>
              </template>
              <div v-else class="r-row r-gcash">
                <span>GCash</span><span>✓ Paid</span>
              </div>
            </div>
            <div class="r-footer">Thank you for your visit! ☕</div>
          </div>
          <div class="receipt-btns">
            <button class="rbtn-back" @click="showReceipt = false">
        ...
        <div v-else class="payment-view">
          ...
          <div class="pm-tabs">
            <button
              :class="['pm-tab', paymentMethod === 'cash' ? 'active' : '']"
              @click="paymentMethod = 'cash'"
            >
              <Banknote :size="17" /> Cash
            </button>
            <button
              :class="['pm-tab', paymentMethod === 'gcash' ? 'active' : '']"
          ...
          <div v-if="paymentMethod === 'cash'" class="cash-fields">
            ...
            <div class="cash-received-display">
              ...
              <div
                class="crd-change"
                :class="changeAmount >= 0 ? 'sufficient' : 'insufficient'"
              >
                <template v-if="cashReceived === 0"></template>
                <template v-else-if="changeAmount < 0">
                  ₱{{ Math.abs(changeAmount).toFixed(2) }} more needed
                </template>
                <template v-else>
            ...
            <div class="denom-section">
              <div class="denom-header-row">
                ...
                <button
                  v-if="cashReceived > 0"
                  class="denom-clear-btn"
                  @click="clearDenomCounts"
                >
              ...
              <div class="denom-grid">
                <button
                  ...
                  :key="d"
                  :class="['denom-btn', denomCounts[d] > 0 ? 'denom-active' : '']"
                  @click="addDenom(d)"
                >
                  <span class="denom-face">
                    ₱{{ d >= 1000 ? (d / 1000) + 'k' : d }}
                  </span>
                  <span class="denom-count" v-if="denomCounts[d] > 0">
                    ×{{ denomCounts[d] }}
                  </span>
                  <span class="denom-badge" v-if="denomCounts[d] > 0">
                    {{ denomCounts[d] }}
                  </span>
                </button>
              </div>
    ...
    <div v-if="showHistory" class="overlay" @click.self="showHistory = false">
      <div class="history-modal">
        ...
        <div v-else-if="transactions.length === 0" class="hm-empty">
          ...
          <table class="hm-table">
            ...
            <tbody>
              <tr
                ...
                :key="tr.OrderId"
                :class="{ 'cancelled-row': tr.Status === 'cancelled' }"
              >
                <td>
                  <span class="id-tag">#{{ tr.transaction_number || tr.OrderId }}</span>
                </td>
                ...
                <td>
                  <div v-if="tr.discount">
                    ...
                    <div v-if="tr.discount_id_number" class="disc-id-display">
                      ID: {{ tr.discount_id_number }}
                    </div>
                  </div>
                ...
                <td>
                  ...
                  <div
                    v-if="tr.Status === 'cancelled' && tr.cancel_reason"
                    class="cancel-reason-display"
                  >
                    "{{ tr.cancel_reason }}"
                  </div>
                </td>
...
// ─── Cache config ─────────────────────────────────────────────────────────────
const CACHE_KEYS = {
  menu: "pos_cache_menu", recipes: "pos_cache_recipes",
  stock: "pos_cache_stock", unitMap: "pos_cache_unitMap",
  discounts: "pos_cache_discounts", transactions: "pos_cache_transactions",
};
const CACHE_TTL = {
  menu: 30 * 60 * 1000, recipes: 5 * 60 * 1000,
  stock: 1 * 60 * 1000, discounts: 10 * 60 * 1000, transactions: 30 * 1000,
};
const readCache = (key) => {
  try {
...
const writeCache = (key, data, ttl) => {
  try { sessionStorage.setItem(key, JSON.stringify({ data, expiresAt: Date.now() + ttl })); } catch {}
};
...
const cashReceived = computed(() =>
  manualOverride.value !== null
    ...
    : denominations.reduce((s, d) => s + denomCounts[d] * d, 0)
);
...
const menuCategories = computed(() =>
  [...new Set(menu.value.map((i) => i.Category).filter(Boolean))].sort()
);
...
const cartSubtotal = computed(() =>
  cart.value.reduce((s, i) => s + i.effectivePrice * i.qty, 0)
);
...
function isProductUnavailable(item) { return getShortages(item.ProductId, 1).length > 0; }
...
function getOOSShortLabel(item) {
  const s = getShortages(item.ProductId, 1);
  if (s.length === 0) return "";
  if (s.length === 1) return s[0].name;
  return `${s[0].name} +${s.length - 1} more`;
}
...
const increaseQty = (i) => {
  ...
  const shortages = getShortages(item.ProductId, item.qty + 1);
  if (shortages.length > 0) {
    oosProductName.value = item.ProductName;
    oosIngredients.value = shortages;
    showOosAlert.value = true;
...
const fetchMenu = async (force = false) => {
  if (!force) {
    ...
    if (cachedMenu && cachedRecipes && cachedStock && cachedUnitMap) {
      ...
      inventoryUnitMap.value = cachedUnitMap;
      return;
    }
  }
  ...
  const { data: products, error } = await supabase.from("product")
    ...
    .neq("Status", "Archived").order("Category").order("ProductName");
...
const fetchDiscounts = async (force = false) => {
  if (!force) { const cached = readCache(CACHE_KEYS.discounts); if (cached) { discounts.value = cached; return; } }
  const { data } = await supabase.from("discount").select("discountid, discountname, discounttype, discountvalue").order("discountname");
  if (data) { discounts.value = data; writeCache(CACHE_KEYS.discounts, discounts.value, CACHE_TTL.discounts); }
};
...
.cash-received-display { background: #f9f4ef; border-radius: 12px; padding: 14px 16px; text-align: center; }
.crd-label { font-size: 11px; font-weight: 700; color: #999; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 4px; }
.crd-amount {
  font-size: 32px;
  font-weight: 800;
  color: #31201d;
...
.crd-amount::-webkit-outer-spin-button,
.crd-amount::-webkit-inner-spin-button {
  -webkit-appearance: none;
}
.crd-amount:focus {
  background: rgba(49,32,29,0.05);
  border-radius: 8px;
}
.crd-change { font-size: 13px; font-weight: 600; }
...
.denom-clear-btn { display: flex; align-items: center; gap: 4px; background: none; border: 1px solid #e8e0d5; color: #999; font-size: 11px; font-weight: 600; padding: 3px 9px; border-radius: 6px; cursor: pointer; font-family: inherit; transition: 0.15s; }
.denom-clear-btn:hover { background: #fee2e2; border-color: #fca5a5; color: #dc2626; }
...
.denom-btn:hover { border-color: #c49a6c; background: #fdf6ee; transform: translateY(-1px); }
.denom-btn.denom-active { border-color: #31201d; background: #fdf6ee; }
.denom-face { font-size: 15px; font-weight: 800; color: #31201d; }
.denom-count { font-size: 11px; font-weight: 600; color: #c49a6c; line-height: 1; }
.denom-badge {
  position: absolute;
  top: -7px;
  right: -7px;
...
/* ─── Exact button ──────────────────────────────────────────────────────────── */
.exact-btn { display: flex; align-items: center; justify-content: center; gap: 7px; width: 100%; padding: 11px; background: #f0fdf4; border: 1.5px solid #86efac; border-radius: 10px; color: #15803d; font-size: 13px; font-weight: 700; cursor: pointer; font-family: inherit; transition: 0.2s; }
